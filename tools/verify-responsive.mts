 ...
 * Run: node --import tsx tools/verify-responsive.mts [--screenshots] [--access-only | --focus-only | --forced-colors-only] [--focus-baseline]
...
import { browserPageUrl, bundleBrowserFixture, connectCdp, delay, evaluate as evaluateInBrowser, serverPort, waitFor, type CdpClient } from "./lib/browser-check.mts";
...
type HorizontalBounds = { x: number; right: number };
type Bounds = HorizontalBounds & { y: number; bottom: number };
type Rect = Bounds & { width: number; height: number };
type HorizontalScroll = { scrollWidth: number; clientWidth: number };
type ScrollDimensions = HorizontalScroll & { scrollHeight: number; clientHeight: number };
type Outsets = Record<"top" | "right" | "bottom" | "left", number>;
type FocusMeasurement = {
  kind: string; focusOutsets: Outsets; [key: string]: unknown;
};
type CheckResult = {
  theme: string; width: number; height?: number; fixture?: string; passed: boolean;
  columns?: number | number[]; tableScrolls?: boolean; toolbarActions?: number;
  forcedColors?: boolean; measurements?: FocusMeasurement[];
};
type BaseGeometry = {
  viewport: number; documentWidth: number; columns: number;
  selects: Array<ScrollDimensions & { rect: Rect }>;
  table: HorizontalScroll & { rect: Rect }; tabs: HorizontalScroll & { rect: Rect };
  toolbar: { rect: Rect; filters: Rect; actions: Rect; buttons: Rect[] };
};
type SelectGeometry = Bounds & HorizontalScroll & { options: HorizontalScroll[] };
type AccessGeometry = {
  documentWidth: number; dialog: Bounds; scroll: Bounds; save: Bounds;
  overflow: ScrollDimensions[]; columns: number[]; groupNames: string[];
  hasUnexpectedIdentifier: boolean; scrolls: boolean; unlabeledCheckboxes: number;
};
type CheckboxState = { checked: boolean; disabled: boolean; mixed: boolean; label: string | null };
type AccessStates = Record<string, { group: CheckboxState; options: CheckboxState[] }>;
type FocusGeometry = ScrollDimensions & {
  kind: string; active: boolean; activeId?: string; rect: Bounds; boundary: Bounds;
  clips: HorizontalBounds[]; boxShadow: string; outlineStyle: string; outlineWidth: string;
  outlineOffset: string; outlineColor: string; ringOffset: string; ringInset: string;
  resize: string; minWidth: string; maxWidth: string; documentWidth: number;
};
...
try {
  ...
  await bundleBrowserFixture(projectRoot, entryPath, join(temporaryRoot, "fixture.js"));
  ...
  server = createServer(async (request, response) => {
    ...
    const files: Record<string, string> = { "/": "index.html", "/fixture.js": "fixture.js", "/fixture.css": "fixture.css" };
  ...
  const pageUrl = `http://127.0.0.1:${serverPort(server)}/`;
  ...
  for (const { theme, width } of browserCases) {
    ...
    await client().send("Page.navigate", { url: caseUrl });
    ...
    await settle();
    const base = await evaluate<BaseGeometry>(`(() => {
  ...
  for (const { theme, width, height } of navigationCases) {
    ...
    const closed = await evaluate<{ triggerVisible: boolean; documentWidth: number; panelPresent: boolean }>(`(() => { const trigger = document.querySelector('[aria-label="Abrir menu"]'); const r = trigger?.getBoundingClientRect(); return { triggerVisible: Boolean(r && r.width && r.height), documentWidth: document.documentElement.scrollWidth, panelPresent: Boolean(document.querySelector('[data-mobile-navigation-panel]')) }; })()`);
    ...
    if (width < 1024) {
      ...
      await settle();
      const overlayPoint = await evaluate<{ x: number; y: number; available: boolean }>(`(() => { const r = document.querySelector('[data-mobile-navigation-panel]').getBoundingClientRect(); return { x: r.right + (innerWidth - r.right) / 2, y: innerHeight / 2, available: r.right < innerWidth - 1 }; })()`);
      ...
      await client().send("Emulation.setDeviceMetricsOverride", { width: 1024, height, deviceScaleFactor: 1, mobile: false });
  ...
  for (const { theme, width, height } of accessCases) {
    ...
    const geometry = await evaluate<AccessGeometry>(`(() => {
    ...
    const changes = await evaluate<number>("window.__accessChanges || 0");
  ...
  for (const { theme, width, height, forcedColors } of focusCases) {
    ...
    await client().send("Emulation.setDeviceMetricsOverride", { width, height, deviceScaleFactor: 1, mobile: false });
    await client().send("Emulation.setEmulatedMedia", { features: [{ name: "forced-colors", value: forcedColors ? "active" : "none" }] });
    ...
    for (const context of ["scroll", "toolbar", "dialog"]) {
      ...
      const ids = await evaluate<string[]>(`Array.from(document.querySelectorAll('[data-focus-control][id^="${context}-"]'), e => e.id)`);
      for (const id of ids) {
        for (const mode of ["keyboard", "pointer"]) {
          ...
          if (mode === "keyboard") {
            ...
            if (before.kind === "select") {
              ...
              await settle();
              const menu = await evaluate<HorizontalBounds & HorizontalScroll>("(() => { const e = document.querySelector('[role=listbox]'); const r = e.getBoundingClientRect(); return { x:r.x, right:r.right, scrollWidth:e.scrollWidth, clientWidth:e.clientWidth }; })()");
          ...
          const focusOutsets = Object.fromEntries((Object.keys(totalPaint) as Array<keyof Outsets>).map((side) => [side, Math.max(0, totalPaint[side] - previousPaint[side])])) as Outsets;
  ...
  if (runningChrome && runningChrome.exitCode === null) {
    runningChrome.kill();
    await Promise.race([new Promise((resolveExit) => runningChrome.once("exit", resolveExit)), delay(2000)]);
  }
  const runningServer = server;
  if (runningServer) await new Promise((resolveClose) => runningServer.close(resolveClose));
...
async function checkScenarioNames(width: number, selector: string, expectedCount: number) {
  const labels = await evaluate<Array<HorizontalBounds & ScrollDimensions>>(`Array.from(document.querySelectorAll(${JSON.stringify(selector)}), e => { const r = e.getBoundingClientRect(); return { x: r.x, right: r.right, scrollWidth: e.scrollWidth, clientWidth: e.clientWidth, scrollHeight: e.scrollHeight, clientHeight: e.clientHeight }; })`);
...
function client(): CdpClient {
  assert.ok(cdp, "Browser debugging must be connected before fixture evaluation.");
  return cdp;
}
async function evaluate<T = unknown>(expression: string): Promise<T> {
  return evaluateInBrowser<T>(client(), expression);
}
async function settle() { await delay(250); }
async function measureFocus(id: string): Promise<FocusGeometry> {
  return evaluate<FocusGeometry>(`(() => {
    const e = document.getElementById(${JSON.stringify(id)});
...
function focusPaintOutsets(boxShadow: string, outlineWidth = "0px", outlineOffset = "0px", outlineStyle = "none", outlineColor = "black"): Outsets {
  const outsets: Outsets = { top: 0, right: 0, bottom: 0, left: 0 };
  const shadows = [];
  let depth = 0;
  let start = 0;
  ...
  if (outlineStyle !== "none" && outlineStyle !== "hidden" && !/transparent|rgba\([^)]*,\s*0(?:\.0+)?\s*\)/.test(outlineColor)) {
    ...
    for (const side of Object.keys(outsets) as Array<keyof Outsets>) outsets[side] = Math.max(outsets[side], extent);
  }
  return outsets;
}
async function readAccessCheckboxes(): Promise<AccessStates> {
  await settle();
  return evaluate<AccessStates>(`Object.fromEntries([...document.querySelectorAll('[data-user-access-group]')].map(section => {
...
async function assertPageInteractive(label: string) {
  const before = await evaluate<number>("window.__outsideClicks || 0");
...
async function screenshot(filename: string, fullPage = false) {
  if (!screenshotDirectory) return;
  await mkdir(screenshotDirectory, { recursive: true });
  const dimensions = fullPage
    ? await evaluate<{ width: number; height: number }>("({ width: innerWidth, height: document.documentElement.scrollHeight })")
    : null;
...
async function click(selector: string) {
  const point = await evaluate<{ x: number; y: number }>(`(() => { const e = document.querySelector(${JSON.stringify(selector)}); e.scrollIntoView({block:'center'}); const r = e.getBoundingClientRect(); return { x: r.x + r.width / 2, y: r.y + r.height / 2 }; })()`);
...
async function key(value: string, code: string, virtualKeyCode: number, modifiers = 0) {
  await client().send("Input.dispatchKeyEvent", { type: "keyDown", key: value, code, modifiers, windowsVirtualKeyCode: virtualKeyCode });
  await client().send("Input.dispatchKeyEvent", { type: "keyUp", key: value, code, modifiers, windowsVirtualKeyCode: virtualKeyCode });
}
function assertInside(rect: HorizontalBounds, width: number, label: string) {
