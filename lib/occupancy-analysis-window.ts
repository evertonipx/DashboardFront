...
export function loadOccupancyAnalysisDateRange(
  ...
  try {
    ...
    const stored = JSON.parse(raw) as Partial<OccupancyAnalysisDateRangeInput>;
    if (
      typeof stored.startInput !== "string" ||
      typeof stored.endInput !== "string" ||
      !parseOccupancyAnalysisDateInput(stored.startInput) ||
      !parseOccupancyAnalysisDateInput(stored.endInput)
    ) {
      return fallback;
    }
    const normalizedStart = normalizeDateInputAgainstMaximum(
