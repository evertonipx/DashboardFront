...
export function loadOccupancyWidgetSettings(
  ...
  try {
    const storageKey = occupancyWidgetSettingsStorageKey(
      companyId,
      userId,
      viewId,
    );
    ...
    const legacyStored =
      !stored && viewId
        ? window.localStorage.getItem(
            occupancyWidgetSettingsStorageKey(companyId, userId),
          )
        : null;
    return stored || legacyStored
      ? normalizeOccupancyWidgetSettings(
          JSON.parse((stored || legacyStored) as string) as unknown,
        )
