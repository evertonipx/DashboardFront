...
export const cardViewMenus: CardMenuDefinition[] = [
  ...
  },
  {
    ...
    cards: [
      ...
      card("occupancy_scenario_max_year", "Máximo por ano por cenário", "Pico observado em cada um dos últimos 4 anos."),
      card("occupancy_duration_average", "Ocupação e permanência", "Ocupação média, tempos da área ocupada e livre, cobertura e permanência média individual concluída por cenário e no consolidado."),
      ...
      card("occupancy_loitering_accumulated_session_time", "Duração acumulada das permanências", "Estimativa por área calculada do resumo completo: média × permanências concluídas. Não representa tempo cronológico de ocupação da área."),
      ...
      card("occupancy_duration_month_heatmap", "Tempo ocupado · dias e horários", "Tempo ocupado por dia e hora desde o início do mês, nos cenários escolhidos."),
      card("occupancy_duration_scenario_heatmap", "Tempo ocupado · cenários e horários", "Comparação do tempo ocupado em cada hora entre os cenários do mês."),
      card("occupancy_duration_week_heatmap", "Tempo ocupado · perfil semanal", "Padrão de ocupação por dia da semana e horário no mês atual."),
      card("occupancy_duration_daily_profile", "Tempo ocupado · evolução diária", "Proporção diária entre tempo ocupado e desocupado confirmado."),
      ...
      card("occupancy_scenario_hour_heatmap", "Ocupação por cenários x períodos", "Mapa comparativo dos cenários na granularidade configurada."),
    ],
  },
];
...
// Old saved views can still pass these IDs as an explicit card list. Keep
// them retired during normalization as well as out of the current catalogue.
const OCCUPANCY_RETIRED_CARD_IDS = new Set([
...
export function normalizeCardPreferences(
  ...
  const requestedCardIds = cardIds?.length
    ? cardIds
    : getCardMenuDefinition(menuKey).cards.map((card) => card.id);
  const definitionIds = new Set(
    requestedCardIds.filter(
      (id) => menuKey !== "occupancy" || !OCCUPANCY_RETIRED_CARD_IDS.has(id),
    ),
  );
  ...
  const normalized = storedOrder.map((id) => {
    ...
    const scenarioOrder = normalizeCardScenarioIds(
      storedPreference.scenarioOrder,
    );
