...
/* The aggregate contract has no pagination or completeness metadata. The
   deployed API caps a response at 1,000 rows, so hitting that boundary is
   treated as potentially incomplete and the interval is divided until every
...
async function fetchHourlyAggregateQuery(
  ...
  for (const partition of partitions) {
    signal?.throwIfAborted();
    completeRows.push(
      ...(await fetchHourlyAggregateQuery(
        partition,
        metricType,
        signal,
        companyScopeId,
  ...
  return requireAggregateRows(completeRows, "hour", metricType);
}
...
function splitHourlyAggregateQuery(
  ...
  return [
    {
      from: query.from,
      key: `${query.key}:before:${boundaryKey}`,
      to: boundary,
    },
    {
      from: boundary,
      key: `${query.key}:from:${boundaryKey}`,
      to: query.to,
    },
  ];
}
