...
test("o ajuste opcional recebe tamanho real depois dos estilos de exportação, sem mutar a origem", () => {
  ...
  for (const size of [{ width: 900, height: 400 }, { width: 520, height: 260 }]) {
    ...
    const result = prepare({
      option,
      fitOption: (prepared: RuntimeFixture, dimensions: RuntimeFixture) => {
        calls++;
...
for (const exporter of ["exportReportToExcel", "exportReportToPdf"]) {
  test(`${exporter}: rasterização usa a opção ajustada, dimensões exatas e o sinal de cancelamento`, async () => {
    ...
    let fitSize: RuntimeFixture;
    let rendered = 0;
    const option = { series: [{ type: "pie", data: [{ name: "Categoria", value: 1 }] }] };
    const result = await render({
      option,
      fitOption: (prepared: RuntimeFixture, size: RuntimeFixture) => {
        fitSize = size;
        return { ...prepared, fittedForExport: true };
      },
    }, { signal: controller.signal }, prepare, async (prepared: RuntimeFixture, size: RuntimeFixture) => {
      rendered++;
...
test("circulares ajustados para PDF/Excel preservam percentuais exatos, fatias e textos em SVG real", () => {
  for (const [dimension, categories] of Object.entries(groups)) for (const type of ["pie", "donut", "half-donut", "rose"]) {
    ...
    assert.deepEqual(prepared.series.flatMap((series: RuntimeFixture) => series.data).map(({ key, value, count, percentage }: RuntimeFixture) => ({ key, value, count, percentage })),
      visible.map(({ key, count, percentage }: RuntimeFixture) => ({ key, value: count, count, percentage })));
...
function declaration(source: RuntimeFixture, name: RuntimeFixture) {
  const found = source.statements.find((node: import("typescript").Node) => ts.isFunctionDeclaration(node) && node.name?.text === name);
  assert.ok(found, `${name} deve existir`);
  return found;
}

function descendants(node: import("typescript").Node) {
  const result: RuntimeFixture[] = [];
  const visit = (child: RuntimeFixture) => { result.push(child); ts.forEachChild(child, visit); };
  visit(node);
  return result;
}

function exportRenderStep(name: RuntimeFixture) {
...
function load(path: string, extraExports: RuntimeFixture[] = []): RuntimeFixture {
