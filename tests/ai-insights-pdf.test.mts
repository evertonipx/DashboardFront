...
const loadTypeScriptModule = createModuleLoader(projectRoot, { compilerOptions: { esModuleInterop: true } });
const pdf = loadTypeScriptModule<typeof import("../lib/ai-insights-pdf.ts")>("lib/ai-insights-pdf.ts");
...
test("composição do IA Advisor separa o cabeçalho e preserva somente conteúdo executivo", () => {
  ...
  const header = pdf.buildAiInsightsPdfHeader(report, headerOptions);
  ...
  const actionHeadings = blocks
    .filter(
      (block): block is Extract<import("../lib/ai-insights-pdf.ts").AiInsightsPdfBlock, {kind: "item-heading"}> =>
...
test("normalização PDF remove NUL e mojibake sem acionar Unicode incompatível", () => {
  ...
  const targetBlock = pdf
    ...
    .find((block): block is Extract<import("../lib/ai-insights-pdf.ts").AiInsightsPdfBlock, {kind: "paragraph"}> => block.kind === "paragraph" && block.label === "Meta do piloto");
...
function validReport(): import("../lib/ai-insights-contract.ts").AiInsightsReport {
  return {
