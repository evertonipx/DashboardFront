...
const fixtureGlobals = globalThis as { window?: RuntimeFixture };
...
const dateRangeWrites: RuntimeFixture[] = [];
const demographicsDateRange = loadTypeScriptModule(
  "lib/demographics-date-range.ts",
  {
    "@/lib/master-company-scope": {
      ...
      readUserViewScopedStorageEntry(baseKey: RuntimeFixture, companyId: string, userId: string, viewId: string) {
    ...
    "@/lib/user-grid-local": {
      writeUserGridPreference(key: string, value: RuntimeFixture) {
...
test("análise vazia permite consultar hoje com o dia civil atual, inclusive após meia-noite", () => {
  ...
  const view = renderDemographicsEmptyState({ applyRange: (range: RuntimeFixture) => calls.push(range) });
...
function renderDemographicsEmptyState(overrides: Record<string, RuntimeFixture> = {}) {
  ...
  const bindings: Record<string, RuntimeFixture> = {
    ...
    Date: class extends Date { constructor(...args: RuntimeFixture[]) { super(...(args.length ? args : ["2026-09-11T03:10:00Z"]) as [string | number | Date]); } },
  ...
  const loaded: { exports: RuntimeFixture } = { exports: null };
  new Function("module", ...Object.keys(bindings), output)(loaded, ...Object.values(bindings));
  return loaded.exports;
}

function findEmptyStateButton(node: RuntimeFixture): RuntimeFixture {
...
test("valida, normaliza e ordena buckets pelo instante e pelas dimensões", () => {
  ...
  assert.deepEqual(
    rows.map(({ bucket, camera_id, age_bucket }: RuntimeFixture) => [
      bucket,
      camera_id,
      age_bucket,
    ]),
...
test("agrega gênero, idade, emoção e todos os cruzamentos disponíveis", () => {
  ...
  assert.deepEqual(
    result.gender.map(({ key, count, percentage, observed }: RuntimeFixture) => ({
      key,
      count,
      percentage,
      observed,
  ...
  assert.deepEqual(
    result.age.map(({ key, count }: RuntimeFixture) => [key, count]),
    [
      ["0-2", 0],
      ["3-9", 0],
      ["10-19", 0],
  ...
  const age20 = result.crossings.ageByGender.rows.find(
    ({ key }: RuntimeFixture) => key === "20-29",
  );
  assert.deepEqual(
    age20.cells.map(({ columnKey, count, observed }: RuntimeFixture) => [
      columnKey,
      count,
      observed,
    ]),
  ...
  const women = result.crossings.genderByEmotion.rows.find(
    ({ key }: RuntimeFixture) => key === "Woman",
  );
  assert.equal(
    women.cells.find(({ columnKey }: RuntimeFixture) => columnKey === "happy").count,
    2,
  );
  assert.equal(
    result.crossings.ageByEmotion.rows
      .find(({ key }: RuntimeFixture) => key === "70+")
      .cells.find(({ columnKey }: RuntimeFixture) => columnKey === "sad").count,
    1,
  );
});
...
test("percentuais canônicos somam exatamente 100 sem divisão por zero", () => {
  ...
  assert.deepEqual(
    result.gender.map(({ percentage }: RuntimeFixture) => percentage),
    [33.34, 33.33, 33.33],
  );
  assert.equal(
    result.gender.reduce((sum: RuntimeFixture, { percentage }: RuntimeFixture) => sum + percentage, 0),
    100,
  );
  for (const distribution of [result.gender, result.age, result.emotion]) {
    assert.equal(
      distribution.reduce((sum: RuntimeFixture, { count }: RuntimeFixture) => sum + count, 0),
      result.total,
      "cada dimensão deve usar exatamente o mesmo denominador",
    );
    assert.equal(
      distribution.reduce((sum: RuntimeFixture, { percentage }: RuntimeFixture) => sum + percentage, 0),
      100,
      "cada distribuição não vazia deve fechar em 100%",
    );
  }
  assert.equal(
    result.gender.find(({ key }: RuntimeFixture) => key === "unknown").percentage,
    33.33,
  ...
  assert.ok(empty.gender.every(({ percentage }: RuntimeFixture) => percentage === null));
...
test("contagens explicitamente zero não fabricam percentuais", () => {
  ...
  assert.ok(result.gender.every(({ percentage }: RuntimeFixture) => percentage === null));
  assert.ok(result.age.every(({ percentage }: RuntimeFixture) => percentage === null));
  assert.ok(result.emotion.every(({ percentage }: RuntimeFixture) => percentage === null));
  assert.equal(
    result.gender.find(({ key }: RuntimeFixture) => key === "Woman").observed,
    true,
  );
  assert.equal(
    result.gender.find(({ key }: RuntimeFixture) => key === "unknown").observed,
    true,
  );
  assert.equal(
    result.gender.find(({ key }: RuntimeFixture) => key === "Man").observed,
    false,
  );
});
...
test("zero explícito é dado observado, mas minuto ausente permanece null", () => {
  ...
  assert.deepEqual(
    timeline.map(({ status, total }: RuntimeFixture) => [status, total]),
    [
      ["observed", 2],
      ["missing", null],
      ["observed", 0],
...
test("timeline consolidada sinaliza buckets parciais e respeita limite", () => {
  ...
  assert.deepEqual(
    timeline.map(
      ({ status, total, observedMinuteCount, expectedMinuteCount }: RuntimeFixture) => [
        status,
        total,
        observedMinuteCount,
        expectedMinuteCount,
...
test("interface demográfica oculta campos e identificadores do contrato interno", () => {
  ...
  invalidPayloads.forEach((payload) => {
    let validationError: RuntimeFixture;
    try {
      demographics.requireDemographicBucketsResponse(payload);
    } catch (error: RuntimeFixture) {
...
function row(overrides: Record<string, RuntimeFixture> = {}) {
  return {
    age_bucket: "20-29",
    bucket: "2026-09-01T14:35:00Z",
    camera_id: CAMERA_A,
...
function scopedStorageKey(baseKey: RuntimeFixture, companyId: string, userId: string, viewId: string) {
  const segments = [
...
function loadTypeScriptModule(relativePath: string, overrides: Record<string, RuntimeFixture> = {}): RuntimeFixture {
  ...
  const loadedModule: { exports: RuntimeFixture } = { exports: {} };
