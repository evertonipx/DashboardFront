...
function evaluate(code: RuntimeFixture, bindings: RuntimeFixture) {
...
function load(path: string): RuntimeFixture {
  ...
  evaluate(readFileSync(resolve(root, path), "utf8"), {
    ...
    require: (name: RuntimeFixture) => name.startsWith("@/") ? load(`${name.slice(2)}.ts`) : require(name),
  });
...
function harness() {
  ...
  function bindings() {
    ...
    return {
      ...
      window: {
        setTimeout: (callback: (...args: RuntimeFixture[]) => RuntimeFixture) => { timers.set(++nextTimer, callback); return nextTimer; },
        clearTimeout: (id: RuntimeFixture) => timers.delete(id),
      },
      abortRequest: (controller: RuntimeFixture) => controller.abort(),
      isAbortError: (error: RuntimeFixture, signal: RuntimeFixture) => signal.aborted || error?.name === "AbortError",
      userFacingErrorMessage: () => "Falha no comparativo",
      setComparisonState: (result: RuntimeFixture) => { state.result = result; },
      loadPartitionedDemographicAggregation: async ({ partitions, signal, companyScopeId }: RuntimeFixture) => {
        state.calls.push({ partitions, signal, companyScopeId });
        if (state.hold) await new Promise<RuntimeFixture>((resolve) => state.pending.push(resolve));
        signal.throwIfAborted();
        if (state.failure) throw new Error("fixture unavailable");
        return demographics.aggregateDemographicBuckets(partitions.map(({ from, to }: RuntimeFixture) => ({
...
test("ocultar aba cancela baseline pendente sem publicar dados ou cache", async () => {
  ...
  fixture.state.pending.splice(0).forEach((resolve: RuntimeFixture) => resolve());
  await fixture.flush();
