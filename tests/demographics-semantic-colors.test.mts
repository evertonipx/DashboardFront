...
function load(path: string): RuntimeFixture {
  ...
  new Function("module", "exports", "require", compiled)(loaded, loaded.exports, (name: RuntimeFixture) => {
...
function rgb(color: string) { return [1, 3, 5].map((start) => parseInt(color.slice(start, start + 2), 16)); }
...
test("as doze opções oferecem pares distintos e variação real de matiz e luminosidade", () => {
  const pairs = presentation.DEMOGRAPHICS_PALETTES.map(({ id }: RuntimeFixture) => presentation.getDemographicGenderPalette(id));
  assert.equal(pairs.length, 12);
  assert.equal(new Set(pairs.map(({ Woman, Man }: RuntimeFixture) => `${Woman}/${Man}`)).size, 12);
  for (const key of ["Woman", "Man"]) {
    const colors = pairs.map((pair: RuntimeFixture) => pair[key]);
    ...
    assert.ok(new Set(hues.map((value: RuntimeFixture) => Math.floor(value / 15))).size >= 5, `${key}: variar famílias de matiz, não apenas versões do mesmo tom`);
    assert.ok(Math.max(...luminances) - Math.min(...luminances) >= 0.2, `${key}: opções claras e profundas precisam ser visualmente distintas`);
    assert.ok(new Set(luminances.map((value: RuntimeFixture) => Math.floor(value * 10))).size >= 3, `${key}: pelo menos três faixas de luminosidade`);
  }
});
...
test("reordenar ou filtrar gêneros nunca remapeia Homem ou unknown para a primeira cor rosa", () => {
  for (const { id } of presentation.DEMOGRAPHICS_PALETTES) {
    ...
    for (const dimension of ["gender", "age-gender"]) {
      for (const keys of [["Woman", "Man", "unknown"], ["unknown", "Man", "Woman"], ["Man"], ["unknown"]] as const) {
    ...
    for (const type of ["bar", "stacked", "pie", "donut", "half-donut", "rose"]) {
      for (const order of ["default", "ascending", "descending"]) {
        for (const items of [categories, categories.filter(({ key }) => key === "Man")]) {
          ...
          const data = option.series.flatMap((series: RuntimeFixture) => series.data);
          const identified = items.filter(({ key }) => key === "Woman" || key === "Man");
          assert.equal(data.length, identified.length);
          assert.ok(data.every(({ key }: RuntimeFixture) => key === "Woman" || key === "Man"));
          ...
          for (const point of data) {
            ...
            const original = items.find(({ key }) => key === point.key);
...
test("as doze paletas etárias possuem nove tons únicos em progressão estrita do mais jovem claro ao mais velho escuro", () => {
  ...
  for (const { id } of presentation.DEMOGRAPHICS_PALETTES) {
    ...
    assert.equal(new Set(colors.map((color: string) => color.toUpperCase())).size, AGE_KEYS.length, `${id}: faixas diferentes não repetem cores`);
...
test("identidade etária depende da chave canônica, não do índice, filtro ou posição ordenada", () => {
  for (const { id } of presentation.DEMOGRAPHICS_PALETTES) {
    ...
    for (const keys of [AGE_KEYS, [...AGE_KEYS].reverse(), ["70+", "3-9", "30-39"], ["50-59"]]) {
  ...
  for (const { id: palette } of presentation.DEMOGRAPHICS_PALETTES) {
    ...
    for (const [type, orientation] of formats) for (const order of ["default", "ascending", "descending"]) {
      for (const items of subsets) for (const theme of ["light", "dark"]) {
        ...
        const ordered = order === "default" ? items : [...items].sort((left: RuntimeFixture, right: RuntimeFixture) => (order === "ascending" ? 1 : -1) * (left.count - right.count));
        assert.deepEqual(data.map((point: RuntimeFixture) => point.key), ordered.map((point: RuntimeFixture) => point.key));
      }
    }
  }
...
function elements(node: RuntimeFixture, predicate: RuntimeFixture): RuntimeFixture[] {
