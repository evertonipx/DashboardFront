...
function loadModule(relativePath: string): Record<string, RuntimeFixture> {
  ...
  const localRequire = (name: string): unknown => name.startsWith("@/")
...
// These tests inspect ECharts' internal model as well as its public SVG output.
type InspectedChart = Pick<import("echarts").ECharts, "renderToSVGString" | "setOption" | "dispose"> & {
  getModel(): RuntimeFixture;
  getViewOfSeriesModel(model: RuntimeFixture): RuntimeFixture;
};
function withChart<T>(option: RuntimeFixture, inspect: (chart: InspectedChart) => T): T {
...
function svg(option: RuntimeFixture) {
  return withChart(option, (chart) => chart.renderToSVGString());
}

function cartesian<T extends unknown[] | undefined>(type: string, data: T, seriesOverrides: Record<string, unknown> = {}, optionOverrides: Record<string, unknown> = {}) {
  return {
    animation: false,
...
test("SVG heatmap: avalia a medida, não as coordenadas zero da célula", () => {
  const option = {
    ...
    series: [{ type: "heatmap", data: [[0, 0, 9], [1, 0, 0], [0, 1, -3]], label: { show: true, formatter: (params: { value: number[] }) => `CELULA=${params.value[2]}` } }],
  };
...
test("formatter callback preserva saída não-zero e os parâmetros reais do ECharts", () => {
  const observed: { value: unknown; name: string; seriesName: string; dataIndex: number }[] = [];
  const formatter = function (params: { value: unknown; name: string; seriesName: string; dataIndex: number }) {
...
test("null, undefined e marcador de ausência não são convertidos em zero", () => {
  const formatter = ({ value }: { value: unknown }) => value == null || value === "-" ? "SEM DADOS" : `VALOR=${value}`;
...
test("tooltip e referência zero em eixo permanecem intactos mesmo com todos os valores zerados", () => {
  const tooltipFormatter = (params: { value: unknown }[]) => `TOTAL=${params[0].value}`;
...
test("aplicação repetida é estável e não modifica opções, dados ou formatter original", () => {
  let calls = 0;
  const formatter = ({ value }: { value: unknown }) => { calls += 1; return `REPETIDO=${value}`; };
...
test("formatter ausente continua usando fallback nativo para valores não-zero", () => {
  ...
  const rendered = svg(transformed);
  const zeros = (markup: string) => (markup.match(/>0<\/text>/g) ?? []).length;
...
test("callback mantém this e argumentos adicionais; somente zero real é suprimido", () => {
  ...
  const formatter = function (this: typeof context, params: { value: unknown }, suffix: typeof extra) {
...
test("zero negativo e strings numéricas zero são ocultos sem apagar frações não-zero", () => {
  const transformed = suppressZeroChartLabels(cartesian("line", [], { label: { show: true, formatter: ({ value }: { value: unknown }) => `N=${value}` } }));
  const formatter = transformed.series[0].label.formatter;
