...
type GraphicChild = { type: string; name?: string; style: { text: string }; shape: unknown };
const palette = {
...
const buildHexOption = standalone(comparisonFile, "buildHexLayoutOption", {
  ...
  occupancyStateLabel: (value: string) => value,
...
test("hexágono zero mantém célula, polígonos, nome, dado e tooltip; só omite cell-value", () => {
  ...
  assert.equal(zero.rendered.children.filter((child: GraphicChild) => child.type === "polygon").length, 2);
  assert.equal(zero.rendered.children.find((child: GraphicChild) => child.name === "cell-name").style.text, "Sala 00");
  assert.equal(zero.rendered.children.some((child: GraphicChild) => child.name === "cell-value"), false);
  assert.equal(positive.rendered.children.find((child: GraphicChild) => child.name === "cell-value").style.text, "7");
  assert.deepEqual(
    zero.rendered.children.filter((child: GraphicChild) => child.type === "polygon").map((child: GraphicChild) => child.shape),
    positive.rendered.children.filter((child: GraphicChild) => child.type === "polygon").map((child: GraphicChild) => child.shape),
  );
...
test("hexágonos preservam estado semântico, ausência de dados e valores não zero", () => {
  ...
  assert.equal(status.rendered.children.find((child: GraphicChild) => child.name === "cell-value").style.text, "DESOCUPADO");
  const unknown = hexFixture(null, "actual", "unknown");
  assert.equal(unknown.rendered.children.find((child: GraphicChild) => child.name === "cell-value").style.text, "SEM DADOS");
  const fractional = hexFixture(0.01);
  assert.equal(fractional.rendered.children.find((child: GraphicChild) => child.name === "cell-value").style.text, "0.01");
  const negativeZero = hexFixture(-0);
  assert.equal(negativeZero.rendered.children.some((child: GraphicChild) => child.name === "cell-value"), false);
});
...
test("duração zero omite somente texto, mantendo o intervalo e sua geometria", () => {
  const createRenderer = standalone("components/app/occupancy-duration-widgets.tsx", "durationTimelineRenderItem", {
    numericValue: (value: unknown) => typeof value === "number" && Number.isFinite(value) ? value : null,
    formatOccupancyDuration: (seconds: number) => `${seconds}s`,
  });
  const render = createRenderer({ color: "#2255aa", border: "#113366", text: "#ffffff" }, "occupied");
  const draw = (seconds: number) => render({ coordSys: { x: 0, y: 0, width: 120, height: 60 } }, {
    value: (dimension: number) => [0, 0, 100, seconds][dimension],
    coord: ([value]: number[]) => [value, 30], size: () => [100, 30],
  });
  const zero = draw(0);
...
function standalone(relativePath: string, name: string, bindings: Record<string, unknown> = {}): RuntimeFixture {
