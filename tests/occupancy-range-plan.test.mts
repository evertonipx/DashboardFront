...
test("intervalos médios usam semanas completas e preservam bordas diárias", () => {
  ...
  assert.deepEqual(
    plan.segments.map((segment: RuntimeFixture) => segment.granularity),
    ["day", "week", "day"],
  );
...
function assertPlanIsContinuous(plan: RuntimeFixture, from: RuntimeFixture, to: RuntimeFixture) {
  assert.equal(plan.segments[0].from.getTime(), from.getTime());
  assert.equal(plan.segments.at(-1).to.getTime(), to.getTime());
  plan.segments.forEach((segment: RuntimeFixture, index: number) => {
...
function localDateParts(date: RuntimeFixture) {
  return [date.getFullYear(), date.getMonth() + 1, date.getDate()];
}

function loadTypeScriptModule(relativePath: string): RuntimeFixture {
