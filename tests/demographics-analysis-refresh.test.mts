...
function compile(text: string, bindings: Record<string, RuntimeFixture> = {}) {
...
function loadModule(path: string, mocks: Record<string, RuntimeFixture> = {}): RuntimeFixture {
  const loaded: { exports: RuntimeFixture } = { exports: {} };
  compile(readFileSync(resolve(root, path), "utf8"), {
    module: loaded,
    exports: loaded.exports,
    require: (name: RuntimeFixture) => {
...
function createFixture({ visible = true, preferencesReady = true, pageVisible = true, online = true, timeZoneReady = true }: Record<string, RuntimeFixture> = {}) {
  ...
  const refs: Record<string, RuntimeFixture> = {
  ...
  class FixtureDate extends Date {
    constructor(...args: RuntimeFixture[]) { super(...(args.length ? args : [state.now]) as [string | number | Date]); }
    static now() { return state.now.getTime(); }
  }
  const timers = new Map();
  let timerId = 0;
  const fakeWindow: RuntimeFixture = {
    setTimeout: (callback: (...args: RuntimeFixture[]) => RuntimeFixture) => { timers.set(++timerId, callback); return timerId; },
    clearTimeout: (id: RuntimeFixture) => { timers.delete(id); },
  };
  const helperNames = [
  ...
  const helpers = compile(
    helperNames.map((name) => functions.get(name)).join("\n") + `\nreturn {${helperNames.join(",")}};`,
    {
      ...
      apiFetch: async (path: string, options: RuntimeFixture) => {
        ...
        if (state.hold) await new Promise<RuntimeFixture>((resolve) => { state.releases.push(resolve); });
  ...
  function bindings() {
    ...
    const dataScopeKey = compile(`return ${initializers.get("dataScopeKey")};`, {
      ...keyBindings, React: { useMemo: (factory: (...args: RuntimeFixture[]) => RuntimeFixture) => factory() },
    });
    return {
      ...
      setRangeState: (update: RuntimeFixture) => { state.rangeState = typeof update === "function" ? update(state.rangeState) : update; },
      setHistoricalQueryScopeKey: (key: string) => { state.requestedKey = key; },
      setClock: (clock: RuntimeFixture) => { state.clock = clock; },
      setRefreshVersion: (update: RuntimeFixture) => { state.refreshVersion = update(state.refreshVersion); },
      setDataState: (data: RuntimeFixture) => { state.data = data; },
      setLastUpdated: (at: RuntimeFixture) => { state.lastUpdated = at; },
      setLoading: (value: RuntimeFixture) => { state.loading = value; },
      setRefreshing: (value: RuntimeFixture) => { state.refreshing = value; },
      setLoadProgress: () => {},
      setError: (value: RuntimeFixture) => { state.error = value; },
      demographicRequestErrorMessage: (error: RuntimeFixture) => error.message,
    };
  }
  let cacheIdentity: RuntimeFixture;
  ...
  async function flush() {
    ...
    await new Promise<RuntimeFixture>((resolve) => setImmediate(resolve));
  }
...
test("Atualizar aborta comparação antes de limpar cache e baseline antigo não repovoa partições", async () => {
  ...
  fixture.state.releases.splice(0).forEach((release: (...args: RuntimeFixture[]) => RuntimeFixture) => release());
  await aborted;
  assert.equal(fixture.refs.partitionCacheRef.current.size, 0);
  assert.equal(fixture.state.data.summary.total, 100);
});
...
test("aborta antes do novo effect e uma resposta antiga não repovoa cache ou resumo", async () => {
  ...
  await fixture.run();
  fixture.state.releases.forEach((release: (...args: RuntimeFixture[]) => RuntimeFixture) => release());
  await fixture.flush();
...
test("retornar a outra empresa exige nova consulta e não restaura o cache da identidade anterior", async () => {
  ...
  assert.deepEqual(fixture.state.calls.map(({ companyScopeId }: RuntimeFixture) => companyScopeId), ["company-a", "company-b", "company-a"]);
  assert.equal(fixture.state.data.summary.total, 40);
...
for (const [property, next] of [["userId", "user-b"], ["companyId", "company-b"], ["timeZone", "UTC"]] as const) {
