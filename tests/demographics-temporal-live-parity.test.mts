...
let comparisonReady: RuntimeFixture;
function visit(node: import("typescript").Node) {
  if (ts.isFunctionDeclaration(node) && node.name) functions.set(node.name.text, node.getText(ast));
  if (ts.isVariableDeclaration(node) && node.initializer && node.name.getText(ast) === "comparisonReady") comparisonReady = node.initializer.getText(ast);
...
for (const [timeZone, from] of [
  ...
  test(`${timeZone}: bootstrap, cauda, promoção, lacuna e releitura equivalem ao resumo direto`, async () => {
    ...
    const cacheRef: { current: RuntimeFixture } = { current: null };
    ...
    for (const minutes of [6, 7, 70, 73, 73, 185, 240]) {
      ...
      const expected = demographics.aggregateDemographicBuckets(harness.state.rows.filter((row: RuntimeFixture) => Date.parse(row.bucket) < to.getTime()), { timeZone });
    ...
    harness.state.rows = harness.state.rows.map((row: RuntimeFixture) => ({ ...row, count: 0 }));
    ...
    assert.ok(zero.temporal.bins.every((bin: RuntimeFixture) => bin.total === 0));
    harness.state.rows = [];
    harness.state.now += 6 * 3_600_000;
    ...
    assert.ok(harness.requests.every((entry: RuntimeFixture) => entry.companyScopeId === "tenant-a"));
  });
}
...
test("comparação aguarda a publicação principal da janela e refresh atuais", () => {
  ...
  const ready = (changes: Record<string, RuntimeFixture> = {}) => execute(`return ${comparisonReady};`, { ...base, ...changes });
...
test("Atualizar aborta baseline sincronamente e resposta antiga não repovoa o cache limpo", async () => {
  ...
  let release: RuntimeFixture;
  harness.state.wait = new Promise<RuntimeFixture>((resolveRequest) => { release = resolveRequest; });
  ...
  const comparisonRequestRef: { current: RuntimeFixture } = { current: controller };
  const comparisonCacheRef: { current: RuntimeFixture } = { current: { summary: "stale" } };
  const liveRetryRef: { current: RuntimeFixture } = { current: { failures: 2, retryAt: Date.now() + 60_000 } };
  const comparisonRetryRef: { current: RuntimeFixture } = { current: { failures: 1, retryAt: Date.now() + 60_000 } };
...
function fixture(from: RuntimeFixture) {
  ...
  const result = execute(`${names.map((name) => functions.get(name)).join("\n")}\nreturn {live:loadLiveDemographicAggregation,partitioned:loadPartitionedDemographicAggregation};`, {
    ...
    apiFetch: async (path: string, options: RuntimeFixture) => {
      ...
      await state.wait;
      return { data: state.rows.filter((row: RuntimeFixture) => Date.parse(row.bucket) >= request.from && Date.parse(row.bucket) < request.to).map((row: RuntimeFixture) => ({ ...row })) };
    },
  });
  return { ...result, state, requests };
}

function execute(text: string, bindings: RuntimeFixture) {
...
function load(path: string): RuntimeFixture {
  ...
  execute(readFileSync(resolve(root, path), "utf8"), { module: loaded, exports: loaded.exports, require: (name: RuntimeFixture) => name.startsWith("@/") ? load(`${name.slice(2)}.ts`) : require(name) });
