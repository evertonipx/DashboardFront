...
function row(bucket: RuntimeFixture, count: RuntimeFixture, overrides: Record<string, RuntimeFixture> = {}) {
...
test("agregação horária é opt-in e mantém apenas os totais/marginais de cada hora observada", () => {
  ...
  for (const bin of summary.temporal.bins) {
    for (const dimension of ["gender", "age", "emotion"]) {
      assert.equal(Object.values<number>(bin[dimension]).reduce((sum, value) => sum + value, 0), bin.total);
    }
  }
});
...
test("dia possui 24 horas, diferencia zero observado/ausência e mantém futuros vazios", () => {
  ...
  assert.deepEqual(result.points.map((point: RuntimeFixture) => point.label), Array.from({ length: 24 }, (_, hour) => `${String(hour).padStart(2, "0")}:00`));
...
test("DST de Nova York: primavera tem 23 horas sem hora inexistente", () => {
  ...
  assert.ok(!result.points.some((point: RuntimeFixture) => point.label.startsWith("02:")));
  assert.equal(result.points.reduce((sum: RuntimeFixture, point: RuntimeFixture) => sum + (point.total ?? 0), 0), 5);
  assert.equal(result.hourProfile[2].total, null);
});
...
test("DST de Nova York: outono mantém duas ocorrências da 01h e perfil soma ambas", () => {
  ...
  assert.deepEqual(summary.temporal.bins.map((bin: RuntimeFixture) => bin.from), ["2026-11-01T05:00:00.000Z", "2026-11-01T06:00:00.000Z"]);
  ...
  assert.deepEqual(result.points.filter((point: RuntimeFixture) => point.label.startsWith("01:")).map((point: RuntimeFixture) => point.label), ["01:00 UTC-04:00", "01:00 UTC-05:00"]);
...
test("Lord Howe: transição de 30 minutos usa fronteiras reais sem misturar ocorrências", () => {
  ...
  assert.deepEqual(summary.temporal.bins.map((bin: RuntimeFixture) => bin.from), ["2026-04-04T14:00:00.000Z", "2026-04-04T15:00:00.000Z"]);
...
test("fronteira diária rápida preserva DST à meia-noite e datas puladas da IANA", () => {
  for (const [timeZone, from, to, expected] of [
    ...
    assert.deepEqual(result.points.map((point: RuntimeFixture) => point.from), expected, timeZone);
  }
});
...
test("dias sem dados ficam null, zeros observados ficam zero e nenhuma hora do fim exclusivo entra", () => {
  ...
  assert.deepEqual(result.points.map((point: RuntimeFixture) => point.total), [0, null, 5]);
  const cropped = plan(summary, { ...day, to: "2026-09-12T03:00:00Z", now: "2027-01-01T00:00:00Z", interval: "day" });
  assert.deepEqual(cropped.points.map((point: RuntimeFixture) => point.total), [0, null]);
});
...
test("datas e perfil não dependem do fuso do processo/navegador", () => {
  ...
  try {
    const snapshots: RuntimeFixture[] = [];
...
test("muitas linhas do mesmo minuto reutilizam um formatter e não fazem trabalho de fuso por linha", () => {
  ...
  let created = 0;
  let formatted = 0;
  Intl.DateTimeFormat = Object.setPrototypeOf(function (...args: ConstructorParameters<typeof Intl.DateTimeFormat>) {
    created += 1;
    ...
    return formatter;
  }, NativeDateTimeFormat) as typeof Intl.DateTimeFormat;
  try {
...
function freeze(value: RuntimeFixture) {
...
function load(path: string): RuntimeFixture {
  ...
  new Function("module", "exports", "require", javascript)(loaded, loaded.exports, (name: RuntimeFixture) => name.startsWith("@/") ? load(`${name.slice(2)}.ts`) : require(name));
