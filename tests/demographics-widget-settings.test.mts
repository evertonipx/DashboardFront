...
visit(ast);
function evaluate(code: RuntimeFixture, bindings: Record<string, RuntimeFixture> = {}) {
...
test("preferências anteriores à hidratação não liberam consultas dos widgets padrão", () => {
  ...
  const state: RuntimeFixture = { readiness: "pending", preference: null, rows: [{ id: selectedId, visible: true }] };
  ...
  const synchronize = () => evaluate(`return (${variables.get("synchronizePreferences").initializer.arguments[0].getText(ast)});`, {
    ...
    setPreferenceState: (value: RuntimeFixture) => { state.preference = value; },
  })();
  ...
  assert.equal(state.preference.value.some((item: RuntimeFixture) => item.visible !== false), false);
...
function callbackHarness(options: Record<string, RuntimeFixture> = {}) {
  const loaded: RuntimeFixture[] = [];
  const saved: RuntimeFixture[] = [];
  const states: RuntimeFixture[] = [];
  const refetches: RuntimeFixture[] = [];
  ...
  const execute = evaluate(`return (${callbackSource});`, {
    ...
    DEMOGRAPHICS_MENU_KEY: menuKey, DEMOGRAPHICS_CARD_IDS: cardIds,
    loadScopedCardPreferences: (...args: RuntimeFixture[]) => { loaded.push(args); return latest; },
    saveCardPreferences: (...args: RuntimeFixture[]) => { saved.push(args); latest = args[1]; },
    setPreferenceState: (value: RuntimeFixture) => states.push(value),
    apiFetch: (...args: RuntimeFixture[]) => refetches.push(args),
    forceRefresh: (...args: RuntimeFixture[]) => refetches.push(args),
    requestFreshData: (...args: RuntimeFixture[]) => refetches.push(args),
    setClock: (...args: RuntimeFixture[]) => refetches.push(args),
    setRefreshVersion: (...args: RuntimeFixture[]) => refetches.push(args),
  });
  return { execute, loaded, saved, states, refetches, setLatest: (value: RuntimeFixture) => { latest = value; } };
}
...
test("callback busca preferências mais recentes e altera somente apresentação do widget escolhido", () => {
  ...
  assert.deepEqual(updated.map((item: RuntimeFixture) => item.id), fresh.map((item) => item.id));
...
test("relatório usa tipos/paletas/ordem selecionados, com emojis desativados apenas na exportação", () => {
  ...
  report.charts.forEach((chart: RuntimeFixture, index: number) => assert.equal(JSON.stringify(chart.option), JSON.stringify(expected[index])));
...
test("opções visuais do relatório não alteram tabelas, totais, métricas ou período", () => {
  ...
  assert.deepEqual(configured.charts.map((chart: RuntimeFixture) => chart.table), legacy.charts.map((chart: RuntimeFixture) => chart.table));
...
test("relatórios configurados e legados omitem gênero desconhecido sem apagar suas detecções", () => {
  ...
  for (const presentations of [undefined, { demographics_gender_mix: settings }]) {
    ...
    assert.deepEqual(report.charts[0].table.rows.map(({ count, percentage }: RuntimeFixture) => [count, percentage]), [[30, "60%"], [20, "40%"]]);
    assert.equal(report.charts[3].table.rows.reduce((total: RuntimeFixture, row: RuntimeFixture) => total + row.total, 0), 50);
    for (const index of [1, 2]) assert.equal(report.charts[index].table.rows.reduce((total: RuntimeFixture, row: RuntimeFixture) => total + row.count, 0), 100);
  }
