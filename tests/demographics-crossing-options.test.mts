...
const defaults = (dimension: RuntimeFixture, changes: Record<string, RuntimeFixture> = {}) => ({ ...presentation.defaultDemographicPresentation(dimension), ...changes });
const crossingFor = (dimension: RuntimeFixture, source = summary) => source.crossings[dimension === "age-gender" ? "ageByGender" : "ageByEmotion"];
...
test("matriz usa as cores selecionadas por gênero e remove preenchimento de células zeradas", () => {
  for (const theme of ["light", "dark"]) {
    ...
    assert.ok(option.visualMap.every((map: RuntimeFixture) => map.show === false && map.dimension === 2 && map.type === "piecewise"));
    assert.deepEqual(option.visualMap.map((map: RuntimeFixture) => map.seriesIndex), [0, 1]);
    assert.ok(option.visualMap.every((map: RuntimeFixture) => map.pieces[0].lte === 0 && map.pieces[0].color === "transparent"));
    assert.equal(option.legend.show, false);
    assert.deepEqual(option.series.map((series: RuntimeFixture) => series.emphasis.itemStyle.borderColor), ["#DB2777", "#2563EB"]);
    assert.deepEqual(option.series.map((series: RuntimeFixture) => series.itemStyle.color), option.visualMap.map((map: RuntimeFixture) => map.pieces[1].color));
    assert.ok(option.series.every((series: RuntimeFixture) => series.type === "heatmap" && !series.itemStyle.decal));
  }
});
...
test("idade por gênero exclui unknown e recalcula base, ordenação e tooltips sem alterar dados brutos", () => {
  ...
  for (const theme of ["light", "dark"]) {
    ...
    const woman = option.series[0].data.find((value: RuntimeFixture) => value[3] > 0);
    const man = option.series[1].data.find((value: RuntimeFixture) => value[3] > 0);
    ...
    assert.equal(option.series.flatMap((series: RuntimeFixture) => series.data).reduce((sum: RuntimeFixture, value: RuntimeFixture) => sum + value[3], 0), 8);
    ...
    assert.equal(emotion.series[0].data.find((value: RuntimeFixture) => value[3] === 12)[2], 60);
...
test("cruzamento sem gêneros identificados mantém duas colunas sem percentuais inventados", () => {
  ...
  for (const theme of ["light", "dark"]) {
    ...
    assert.deepEqual(option.xAxis.data, ["Mulher", "Homem"]);
    assert.ok(option.series.every((series: RuntimeFixture) => series.data.every((value: RuntimeFixture) => value[2] === null && value[3] === 0)));
...
test("ordenação crescente e decrescente é estável, mantém os dados e corrige tooltips", () => {
  for (const dimension of ["age-gender", "age-emotion"]) {
    ...
    for (const order of ["default", "ascending", "descending"]) {
      ...
      const expected = crossing.rows.map((row: RuntimeFixture, index: number) => ({ row, index }));
      if (order !== "default") expected.sort((a: RuntimeFixture, b: RuntimeFixture) => (order === "ascending" ? 1 : -1) * (a.row.count - b.row.count) || a.index - b.index);
      assert.deepEqual(option.yAxis.data, expected.map(({ row }: RuntimeFixture) => row.label));
      for (const series of option.series) {
        for (const value of series.data) {
          ...
          const cell = row.cells.find((candidate: RuntimeFixture) => candidate.columnKey === column.key);
...
test("heatmap mantém a progressão tonal claro para escuro nos dois temas", () => {
  for (const { id: palette } of presentation.DEMOGRAPHICS_PALETTES) {
    for (const theme of ["light", "dark"]) {
      ...
      assert.equal(option.visualMap.max, Math.max(1, ...option.series[0].data.map((value: RuntimeFixture) => value[2])));
...
test("todas as paletas têm base cromática discreta sem cinza, inversão de intensidade ou mudança dos dados", () => {
  ...
  for (const { id: palette } of presentation.DEMOGRAPHICS_PALETTES) {
    ...
    assert.deepEqual(dark.series.map((series: RuntimeFixture) => series.data), light.series.map((series: RuntimeFixture) => series.data));
...
test("percentuais e counts não são arredondados nos dados; zero/null não produzem rótulo nos dois temas", () => {
  ...
  for (const theme of ["light", "dark"]) for (const dimension of ["age-gender", "age-emotion"]) {
    ...
    assert.ok(emptyOption.series.every((series: RuntimeFixture) => series.data.every((value: RuntimeFixture) => value[2] === null)));
      ...
      for (const { id: palette } of presentation.DEMOGRAPHICS_PALETTES) {
        ...
        try {
          ...
          if (dimension === "age-emotion") {
            ...
            option.series[0].data.forEach((value: RuntimeFixture, index: number) => {
            ...
            option.series.forEach((series: RuntimeFixture, seriesIndex: RuntimeFixture) => {
              const data = chart.getModel().getSeriesByIndex(seriesIndex).getData();
              series.data.forEach((value: RuntimeFixture, index: number) => {
                if (typeof value[2] !== "number") return;
                const graphic = data.getItemGraphicEl(index);
                assert.ok(graphic);
...
function luminance(color: string) {
  const [red, green, blue] = echarts.color.parse(color).slice(0, 3).map((channel: number) => {
