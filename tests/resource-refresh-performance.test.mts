...
const resources: { exports: RuntimeFixture } = { exports: {} };
...
test("hook consolida focus, visibility e timer sem consultas simultâneas", async () => {
  let complete!: () => void;
  const fixture = mountRefresh(() => new Promise<void>((resolve) => { complete = resolve; }));
  fixture.focus();
  fixture.visibility();
  await fixture.flush();
...
function mountRefresh(refresh: () => void | Promise<void>, options?: { enabled?: boolean; intervalMs?: number }) {
  const effects: (() => (() => void) | undefined)[] = [];
  const timers = new Map<number, { callback: () => void; delay: number }>();
  let now = 0;
  let calls = 0;
  const window = Object.assign(new EventTarget(), {
    setInterval(callback: () => void, delay: number) { const id = timers.size + 1; timers.set(id, { callback, delay }); return id; },
    clearInterval(id: number) { timers.delete(id); },
  });
  const document = Object.assign(new EventTarget(), { visibilityState: "visible" });
  const navigator = { onLine: true };
  const React = { useRef: <T,>(value: T) => ({ current: value }), useEffect: (effect: () => (() => void) | undefined) => effects.push(effect) };
  const hookModule: { exports: RuntimeFixture } = { exports: {} };
  new Function("module", "exports", "require", "window", "document", "navigator", "Date", compile("components/app/use-resource-auto-refresh.ts"))(
    ...
    (id: string) => id === "react" ? React : resources.exports,
