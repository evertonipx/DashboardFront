...
function module(family: string) {
  return { id: `module-${family}`, name: family, slug: family, active: true };
}
function grant(slug: string, overrides = {}) {
  return { id: `permission-${slug}`, slug, ...overrides };
}
function user(grants: RuntimeFixture, overrides = {}) {
...
function dashboardViewRoute(currentUser: RuntimeFixture, companyModules?: RuntimeFixture) {
  let reads = 0;
  const paths: string[] = [];
  const mocks = {
    fs: { promises: { readFile: async () => { reads += 1; return "{}"; } } },
    "@/lib/backend-routing": { resolveBackendBaseUrl: () => "https://backend.example.test" },
    "@/lib/access-token-claims": { reconcileCurrentUserWithAccessToken: (value: RuntimeFixture) => value },
    "@/lib/view-preferences": { normalizeCardPreferences: (_key: RuntimeFixture, values: RuntimeFixture) => values },
  };
  const route = loadTypeScriptModule("app/api/v1/dashboard-views/[menuKey]/route.ts", {
    cache: new Map(), mocks,
    fetch: async (url: RuntimeFixture) => {
...
function loadTypeScriptModule(relativePath: string, options: {
  cache?: Map<string, { exports: RuntimeFixture }>;
