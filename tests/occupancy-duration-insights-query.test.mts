...
test("mínimo zero com pico positivo refina em lotes de até 480 minutos", async () => {
  const fixture = createFixture((call) => reply(call, (bucket: RuntimeFixture) => call.granularity === "hour"
...
test("cenário, fuso e buckets fora do filtro são recusados", async () => {
  for (const tamper of [
    (response: RuntimeFixture) => { response.scenario_id = "outside-scope"; },
    (response: RuntimeFixture) => { response.timezone = "America/New_York"; },
    (response: RuntimeFixture) => { response.data[0].bucket = "2020-09-01T00:00:00Z"; },
  ]) {
    const fixture = createFixture((call) => {
...
test("fuso fracionário preserva início civil e divide bordas em minutos", async () => {
  ...
  assert.ok(result.hours.every((hour: RuntimeFixture) => hour.dateKey === "2026-09-01" || hour.dateKey === "2026-09-02"));
  assert.equal(result.hours.filter((hour: RuntimeFixture) => hour.dateKey === "2026-09-01" && hour.hour === 0)
    .reduce((sum: RuntimeFixture, hour: RuntimeFixture) => sum + hour.expectedSeconds, 0), 3_600);
});
...
test("dia de transição DST mantém as duas ocorrências da hora civil", async () => {
  ...
  assert.equal(result.hours.filter((hour: RuntimeFixture) => hour.hour === 1)
    .reduce((sum: RuntimeFixture, hour: RuntimeFixture) => sum + hour.expectedSeconds, 0), 2 * 3_600);
});
...
test("fuso do navegador diferente não desalinha os buckets da empresa", async () => {
  ...
  try {
    ...
    assert.deepEqual(result.hours.map((hour: RuntimeFixture) => hour.hour), [0, 1, 2]);
  } finally {
...
function total(scenario: RuntimeFixture, field: RuntimeFixture) {
  return scenario.hours.reduce((sum: RuntimeFixture, hour: RuntimeFixture) => sum + hour[field], 0);
}

function reply(call: RuntimeFixture, metric: (bucket: number) => { minimum: number | null; average: number | null; peak: number | null } = () => ({ minimum: 1, average: 1, peak: 1 }), options: Record<string, RuntimeFixture> = {}) {
...
function createFixture(responder = reply) {
  ...
  const apiFetch = async (path: string, options: RuntimeFixture) => {
  ...
  const cache = new Map();
  const month = (date: RuntimeFixture, zone = "America/Sao_Paulo") => model.buildOccupancyDurationInsightMonth(new Date(date), zone);
  return {
    cache,
    calls,
    fetch: (overrides: Record<string, RuntimeFixture> = {}) => query.fetchOccupancyDurationInsightScenario({
      cache,
  ...
  function load(relativePath: string): RuntimeFixture {
    ...
    const localRequire = (specifier: string) => specifier === "@/lib/api"
      ? { apiFetch }
