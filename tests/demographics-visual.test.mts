...
function loadModule(path: string): RuntimeFixture {
  ...
  new Function("module", "exports", "require", output)(loaded, loaded.exports, (name: RuntimeFixture) => name.startsWith("@/") ? loadModule(`${name.slice(2)}.ts`) : require(name));
...
function renderPresentationControls(dimension: RuntimeFixture, value: RuntimeFixture) {
  ...
  const controlsModule: { exports: RuntimeFixture } = { exports: {} };
  ...
  const tree = controlsModule.exports({ dimension, value, onChange: (next: RuntimeFixture) => changes.push(next) });
  const elements: RuntimeFixture[] = [];
  function visit(node: RuntimeFixture) {
...
test("a fixture utiliza a lista real de quatro KPIs e cinco gráficos, sem API ou sessão", () => {
  ...
  const tool = readFileSync(resolve(root, "tools/verify-demographics.mts"), "utf8");
...
test("a inspeção visual distingue colisões reais de caixas delimitadoras de textos inclinados", () => {
  const tilted = (offset: number, text: string) => {
  ...
  try {
    chart.setOption({
      ...
      yAxis: { type: "category", data: ["10-19", "Desprezo"], name: "Nome do eixo", axisLabel: {
        interval: 0, fontSize: 11, formatter: (value: RuntimeFixture) => `{label|${value}}{icon| 👩}`,
  ...
  try {
    chart.setOption({
      ...
      legend: {
        ...
        formatter: (name: RuntimeFixture) => `{name|${name}}\n{value|${name.startsWith("Homem") ? "60" : "40"}%}`,
...
test("contenção circular considera o furo da rosca e contraste de texto pequeno", () => {
  ...
  const lightContrast = chartTextContrast("#FFFFFF", "#8A99AF");
  const darkContrast = chartTextContrast("#172033", "#8A99AF");
  assert.ok(lightContrast !== null && darkContrast !== null, "cores válidas devem produzir contraste");
  assert.ok(lightContrast < 4.5);
  assert.ok(darkContrast >= 4.5);
});
...
test("heatmap compacto mantém texto primeiro e emoji à direita sem alterar categorias ou valores", () => {
  ...
  assert.deepEqual(compact.series.map((series: RuntimeFixture) => series.data), original.series.map((series: RuntimeFixture) => series.data));
...
for (const theme of ["light", "dark"]) {
  test(`${theme}: novos formatos renderizam a distribuição real sem texturas nem alterar o total`, () => {
    ...
    for (const [name, preset] of Object.entries(demographicFixturePresets)) {
      for (const dimension of ["gender", "age", "emotion"] as const) {
        ...
        try {
          ...
          if (["pie", "donut", "half-donut", "rose"].includes(settings.type)) {
            ...
            assert.equal(series.data.reduce((total: RuntimeFixture, item: RuntimeFixture) => total + item.value, 0), visible.reduce((total: RuntimeFixture, item: RuntimeFixture) => total + item.count, 0));
...
test("cores etárias reais seguem nove tons monotônicos por chave em toda paleta, formato e ordenação", () => {
  ...
  for (const { id } of presentation.DEMOGRAPHICS_PALETTES) {
    const expected = Object.fromEntries(demographics.AGE_LABELS.map((key: string, index: number) => [key, presentation.demographicCategoryColor(key, index, id, "age")]));
    ...
    const filtered = summary.age.filter((_: RuntimeFixture, index: number) => index % 3 === 1).reverse();
    const chart = echarts.init(null, null, { renderer: "svg", ssr: true, width: 400, height: 240 });
    try {
...
test("detector etário reprova troca de cor por índice mesmo quando a sequência visual parece válida", () => {
  ...
  assert.deepEqual(ageCategoryColorIssues([...actual].reverse(), expected), []);
...
test("séries temporais etárias mantêm a mesma cor por idade após ocultar categorias", () => {
  ...
  for (const paletteId of ["pink-blue", "cyber"]) {
    for (const categoryKeys of [[], ["60-69", "3-9", "20-29"]] as const) {
      const expected = Object.fromEntries(demographics.AGE_LABELS.map((key: string, index: number) => [key, presentation.demographicCategoryColor(key, index, paletteId, "age")]));
...
for (const theme of ["light", "dark"]) {
  ...
  test(`${theme}: densidades compacta e estreita preservam séries, dados e tooltips`, () => {
    const summary = demographics.aggregateDemographicBuckets(demographicFixtureRows);
    for (const [kind, option] of options(summary, theme)) {
      const original = option.series.map((series: RuntimeFixture) => JSON.stringify(series.data));
      for (const density of [{ compact: true, narrow: true }, { compact: false, narrow: true }]) {
        ...
        assert.deepEqual(adapted.series.map((series: RuntimeFixture) => JSON.stringify(series.data)), original);
        assert.equal(adapted.tooltip, option.tooltip);
        assert.notEqual(adapted.aria?.decal?.show, true);
        assert.ok(adapted.series.every((series: RuntimeFixture) => !series.label?.fontSize || series.label.fontSize >= 9));
      }
      assert.deepEqual(option.series.map((series: RuntimeFixture) => JSON.stringify(series.data)), original);
    }
  });
}
...
test("detector lê o traço da legenda de linha e o fundo real dos percentuais", () => {
  ...
  try {
    ...
    assert.ok(actual.labels, "a inspeção da área deve incluir os rótulos renderizados");
