...
const loadTypeScriptModule = createModuleLoader(projectRoot);
const userGrid = loadTypeScriptModule("lib/user-grid.ts");
const userGridLocal = loadTypeScriptModule("lib/user-grid-local.ts");

// The suite installs intentionally partial browser globals, then restores them.
const browserFixture = globalThis as unknown as Record<string, RuntimeFixture>;
type RecordedRequest = { body: RuntimeFixture; method: string; url: string };
type GridDocumentFixture = {
  entries: Record<string, RuntimeFixture>;
  format: string;
  updatedAt: string;
  version: number;
  quarantinedEntries?: Record<string, unknown>;
};
type BrowserContext = {
  requests: RecordedRequest[];
  storage: ReturnType<typeof memoryStorage>;
  timeoutDelays: (number | undefined)[];
...
test("GET do grid falhou: preferências locais nunca habilitam PUT", async () => {
  await withBrowser(
    ...
    async ({ requests, storage }) => {
      ...
      assert.deepEqual(
        requests.map((request: RuntimeFixture) => request.method),
        ["GET"],
      );
    },
  );
...
test("merge do grid isola usuários, preserva empresas e não exclui chaves ausentes", async () => {
  ...
  await withBrowser(
    ...
    async ({ requests, storage }) => {
      ...
      const hydrationPut = requests.find((request: RuntimeFixture) => request.method === "PUT");
...
test("grid legado migra somente preferências pessoais já escopadas", async () => {
  ...
  await withBrowser(
    ...
    async ({ requests }) => {
      ...
      const migrationPut = requests.find((request: RuntimeFixture) => request.method === "PUT");
...
test("documentos v1 em JSON string e byte[] migram para v2 com metadado por entrada", async () => {
  ...
  for (const grid of variants) {
    await withBrowser({}, async ({ requests, storage }) => {
      ...
      const put = requests.find((request: RuntimeFixture) => request.method === "PUT");
      ...
      assert.deepEqual(
        requests.slice(-3).map((request: RuntimeFixture) => request.method),
        ["GET", "PUT", "GET"],
      );
    });
  }
...
test("reconciliação converge após overwrite concorrente e desempata timestamps iguais", async () => {
  ...
  await withBrowser({}, async ({ requests, storage, windowTarget }) => {
    ...
    const getCountBefore = requests.filter((request: RuntimeFixture) => request.method === "GET").length;
    windowTarget.dispatchEvent(new Event("focus"));
    await waitFor(
      () => requests.filter((request: RuntimeFixture) => request.method === "GET").length > getCountBefore,
    );
...
test("documento futuro não suportado e chaves fora da allowlist nunca são enviados", async () => {
  ...
  await withBrowser(
    ...
    async ({ requests, storage }) => {
      ...
      userGrid.requestUserGridSync();
      assert.equal(await userGrid.flushUserGridSync(), false);
      assert.deepEqual(requests.map((request: RuntimeFixture) => request.method), ["GET"]);
...
test("GET inicial falho reintenta com backoff e eventos online/focus reconciliam", async () => {
  ...
  await withBrowser({}, async ({ requests, storage, timeoutDelays, windowTarget }) => {
    const storageEvents: { key: string | null; newValue: string | null }[] = [];
    windowTarget.addEventListener("storage", (event: RuntimeFixture) => {
    ...
    assert.equal(requests.filter((request: RuntimeFixture) => request.method === "GET").length, 2);
    ...
    assert.equal(requests.filter((request: RuntimeFixture) => request.method === "GET").length, 3);
    assert.deepEqual(
...
test("entrada v2 gerenciada malformada é quarentenada e reparada", async () => {
  ...
  await withBrowser({ [key]: "local-safe" }, async ({ requests, storage }) => {
    ...
    assert.ok(requests.some((request: RuntimeFixture) => request.method === "PUT"));
  });
});
...
async function withBrowser(initialStorage: Record<string, unknown>, run: (context: BrowserContext) => void | Promise<void>) {
  const previousWindow = browserFixture.window;
  const previousDocument = browserFixture.document;
  const previousFetch = globalThis.fetch;
  const previousCustomEvent = browserFixture.CustomEvent;
  const previousStorageEvent = browserFixture.StorageEvent;
  const storage = memoryStorage(initialStorage);
  const requests: RecordedRequest[] = [];
  const timeoutDelays: (number | undefined)[] = [];
  const windowTarget = new EventTarget();
  const documentTarget = new EventTarget();

  browserFixture.CustomEvent = class TestCustomEvent extends Event {
    detail: unknown;
  ...
  browserFixture.StorageEvent = class TestStorageEvent extends Event {
  ...
  try {
    ...
    browserFixture.document = previousDocument;
    globalThis.fetch = previousFetch;
    browserFixture.CustomEvent = previousCustomEvent;
    browserFixture.StorageEvent = previousStorageEvent;
  }
}

function scopedKey(baseKey: string, companyId: string, userId: string) {
...
function gridDocument(entries: Record<string, RuntimeFixture>): GridDocumentFixture {
  return {
    entries,
    format: "ipxdata-user-grid",
...
function gridDocumentV2(entries: Record<string, RuntimeFixture>, extras: Record<string, unknown> = {}): GridDocumentFixture {
  return {
    ...extras,
    entries,
    format: "ipxdata-user-grid",
...
function valueEntry(value: unknown, updatedAt: string) {
...
function requestRecord(url: RequestInfo | URL, init: RequestInit = {}): RecordedRequest {
  return {
...
function memoryStorage(initialValues: Record<string, unknown> = {}) {
