...
test("sublocais limitam concorrência a quatro preservando ordem, empresa e sinal", async () => {
  ...
  const calls: { path: string; options: { companyScopeId: string; signal: AbortSignal } }[] = [];
  const fetchSubLocations = standalone("fetchSubLocations", {
    apiFetch: async (path: string, options: RuntimeFixture) => {
    ...
    requireSubLocationRows: (rows: RuntimeFixture) => rows,
    filterScopedApiRows: (rows: RuntimeFixture) => rows,
  });
  ...
  assert.deepEqual(rows.map((row: RuntimeFixture) => row.id), locations.map((location) => location.id));
...
test("replay de montagem agenda somente um catálogo e cleanup cancela seu contexto", () => {
  ...
  const metadataAbortControllerRef: { current: AbortController | null } = { current: null };
...
test("retomar aba respeita frescor, não força requisição concorrente e pausa offline", () => {
  ...
  const setup = effect("function handleVisibilityChange()", {
    ...
    Date: { now: () => now },
    loadCharts: (options: RuntimeFixture) => calls.push(options),
...
test("controles de fontes ocultas não entram nas dependências de rede", () => {
  const load = findNode((node) => ts.isVariableDeclaration(node) && node.name.getText(sourceFile) === "loadCharts") as ts.VariableDeclaration & { initializer: ts.CallExpression };
...
test("resposta reconciliada depois de cancelamento não publica gráficos antigos", async () => {
  ...
  const requestRef: { current: AbortController | null } = { current: null };
  ...
  const bindings = {
    ...
    dedupeRealtimeDefinitions: (definitions: RuntimeFixture) => [...new Map(definitions.map((definition: RuntimeFixture) => [definition.id, definition])).values()],
    ...
    hydrateRealtimeOpenBuckets: (data: RuntimeFixture) => data,
    refreshMinuteDayAggregateCache: () => { entered.resolve(); return reconciliation.promise; },
    setChartData: (data: RuntimeFixture) => published.push(data), setChartLoadError: noop, setClock: noop, setLastUpdated: noop, setHasLoadedCharts: noop,
    React: { startTransition: (callback: RuntimeFixture) => callback() }, toast: { error: noop }, isAbortError: (error: RuntimeFixture) => error?.name === "AbortError", dashboardErrorMessage: (error: RuntimeFixture) => error.message,
  };
...
function deferred() {
  ...
  const promise = new Promise<unknown>((resolveValue) => { resolvePromise = resolveValue; });
...
function fakeTarget(extra = {}) {
  ...
  return {
    ...extra,
    addEventListener: (type: string, callback: RuntimeFixture) => { if (!listeners.has(type)) listeners.set(type, new Set()); listeners.get(type).add(callback); },
    removeEventListener: (type: string, callback: RuntimeFixture) => listeners.get(type)?.delete(callback),
    emit: (type: string, extraEvent = {}) => listeners.get(type)?.forEach((callback: RuntimeFixture) => callback({ type, ...extraEvent })),
  };
}
function fakeWindow() {
  let id = 0;
  ...
  return {
    ...
    setInterval: (callback: () => void) => { intervals.set(++id, callback); return id; },
...
function compile(expression: string, bindings: Record<string, unknown>) {
...
function standalone(name: string, bindings: Record<string, unknown>) {
...
function callback(name: string, bindings: Record<string, unknown>) {
  const node = findNode((item) => ts.isVariableDeclaration(item) && item.name.getText(sourceFile) === name) as ts.VariableDeclaration & { initializer: ts.CallExpression };
...
function effect(marker: string, bindings: Record<string, unknown>) {
  const node = findNode((item) => ts.isCallExpression(item) && item.expression.getText(sourceFile) === "React.useEffect" && !!item.arguments[0]?.getText(sourceFile).includes(marker)) as ts.CallExpression;
