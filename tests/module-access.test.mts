...
test("Operador nunca gerencia recursos mesmo com todos os grants explícitos", () => {
  const operator = user({
    role: "operator",
    permissions: permissions.OPERATIONAL_PERMISSIONS.map(({ slug }: RuntimeFixture) =>
...
test("Master reconhece Demográfico pelos mesmos metadados usados nas permissões", () => {
  ...
  assert.deepEqual(rows.map((row: RuntimeFixture) => row.id), [counting.id, occupancy.id, demographic.id]);
...
function permission(overrides: Record<string, RuntimeFixture> = {}): Record<string, RuntimeFixture> {
  return {
...
function loadMasterModulePresentation() {
  ...
  const statements = source.statements.filter((node) =>
    (ts.isFunctionDeclaration(node) && node.name !== undefined && names.has(node.name.text)) ||
