...
const audit = loadTypeScriptModule<typeof import("../lib/audit.ts")>("lib/audit.ts");
...
test("redação defensiva suporta objetos cíclicos", () => {
  const cyclic: { safe: boolean; self?: unknown } = { safe: true };
  cyclic.self = cyclic;
  const redacted = audit.redactAuditValue(cyclic);

  assert.ok(redacted && typeof redacted === "object" && "safe" in redacted && "self" in redacted);
