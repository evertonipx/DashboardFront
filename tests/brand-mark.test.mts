...
const render = (props: React.ComponentProps<typeof IPXBrandMark> = {}) => renderToStaticMarkup(React.createElement(IPXBrandMark, props));
const markup = render();
const styles = markup.match(/<style>([\s\S]*?)<\/style>/);
assert.ok(styles, "O logo precisa conter seus estilos encapsulados.");
const css = postcss.parse(styles[1]);
...
test("CSS é restrito ao logo e usa movimento finito sem piscar, girar ou mudar o layout", () => {
  ...
  css.walkDecls("transform", (declaration) => {
    ...
    assert.ok(parent && parent.type === "rule");
    const keyframe = parent.parent;
    assert.ok(keyframe);
    assert.ok(parent.selector.endsWith(".ipx-mark-sweep") ||
...
test("preferência de movimento reduzido desativa a animação e alto contraste mantém o monograma", () => {
  const media = css.nodes.filter((node): node is postcss.AtRule => node.type === "atrule" && node.name === "media");
