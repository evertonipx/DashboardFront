...
// These fixtures execute transpiled modules and extracted functions, including
// deliberately malformed payloads. Keep their dynamic boundary local to tests.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type DynamicFixture = any;
type RuntimeModule = { exports: Record<string, DynamicFixture> };
type ExtractedFunction = (...args: DynamicFixture[]) => DynamicFixture;
type AggregateFixtureRow = {
  bucket: string;
  camera_id: string;
...
type OccupancyFixturePoint = { entries: number; exits: number; occupancy: number };
type AnalysisFixtureData = {
  baseline?: Record<string, DynamicFixture>;
  baselineComparable?: Record<string, DynamicFixture>;
  dayRows?: AggregateFixtureRow[];
  hourRows?: AggregateFixtureRow[];
  minuteRows?: AggregateFixtureRow[];
  monthRows?: AggregateFixtureRow[];
};
// Browser globals are replaced by intentionally partial storage/event doubles.
const browserFixture = globalThis as unknown as {
  window?: DynamicFixture;
  CustomEvent?: DynamicFixture;
};
...
test("comparação de ocupação usa o estado explícito sem rederivá-lo do total", () => {
  ...
  assert.deepEqual(
    occupancyComparison
      ...
      .map((snapshot: DynamicFixture) => snapshot.scenarioId),
    ["a"],
  );
  assert.deepEqual(
    occupancyComparison
...
test("meia rosca alterna estado e ocupação real sem apagar cenário zero", () => {
  ...
  assert.deepEqual(
    status.map((entry: DynamicFixture) => [entry.scenarioId, entry.chartValue, entry.state, entry.total]),
    [
      ["free", 1, "unoccupied", 0],
      ["busy", 1, "occupied", 7],
    ],
  ...
  assert.deepEqual(
    actual.map((entry: DynamicFixture) => [entry.scenarioId, entry.chartValue, entry.total]),
    [
      ["free", 0, 0],
      ["busy", 7, 7],
    ],
  );
  assert.equal(actual.some((entry: DynamicFixture) => entry.scenarioId === "missing"), false);
  ...
  assert.deepEqual(
    afterValueChange.map((entry: DynamicFixture) => entry.scenarioId),
    ["free", "busy"],
...
test("barras atuais preservam ordem, zero e ausência nos dois modos", () => {
  ...
  assert.deepEqual(
    actual.map((entry: DynamicFixture) => [entry.scenarioId, entry.chartValue, entry.state, entry.total]),
    [
      ["free", 0, "unoccupied", 0],
...
test("layout hexagonal preserva células, lacunas e troca posições sem colisão", () => {
  ...
  assert.equal(new Set(normalized.cells.map((cell: DynamicFixture) => `${cell.column}:${cell.row}`)).size, 3);
  assert.equal(normalized.cells[2].scenarioId, null);
  const moved = occupancyHexLayout.moveOccupancyHexCell(normalized, "a", 2, 1);
  assert.deepEqual(
    moved.cells.map((cell: DynamicFixture) => [cell.id, cell.column, cell.row]),
    [
      ["a", 2, 1],
      ["b", 1, 0],
      ["c", 0, 0],
  ...
  assert.equal(new Set(resized.cells.map((cell: DynamicFixture) => `${cell.column}:${cell.row}`)).size, 3);
...
test("layout hexagonal escala em lote para 40, 100 e 300 posições sem perder células", () => {
  ...
  assert.equal(
    new Set(parking.layout.cells.map((cell: DynamicFixture) => `${cell.column}:${cell.row}`))
      .size,
    300,
  );
  assert.equal(
    new Set(parking.layout.cells.map((cell: DynamicFixture) => cell.id)).size,
    300,
  );
...
test("layout hexagonal vincula cenários em lote usando reservas antes de criar células", () => {
  ...
  const reservedIds = layout.cells.slice(1).map((cell: DynamicFixture) => cell.id);
  const result =
  ...
  assert.deepEqual(
    result.layout.cells.slice(1, 4).map((cell: DynamicFixture) => cell.id),
    reservedIds,
  );
  assert.deepEqual(
    result.layout.cells.map((cell: DynamicFixture) => cell.scenarioId),
...
test("validação do editor hexagonal relata conflitos sem alterar o documento", () => {
  ...
  assert.equal(
    result.errors.filter((issue: DynamicFixture) => issue.code === "duplicate-cell-id").length,
    1,
  );
  assert.equal(
    result.errors.filter((issue: DynamicFixture) => issue.code === "duplicate-coordinate").length,
    1,
  );
  assert.equal(
    result.errors.filter((issue: DynamicFixture) => issue.code === "duplicate-scenario").length,
    1,
  );
  assert.equal(
    result.errors.filter((issue: DynamicFixture) => issue.code === "cell-out-of-grid").length,
    1,
  );
  assert.equal(
    result.errors.filter((issue: DynamicFixture) => issue.code === "invalid-capacity").length,
    4,
  );
  assert.deepEqual(
    result.warnings.map((issue: DynamicFixture) => issue.code).sort(),
...
test("escala hexbin usa domínio certificado comum e raio monotônico", () => {
  ...
  const byId = new Map<string, DynamicFixture>(visual.entries.map((entry: DynamicFixture) => [entry.cellId, entry]));
  assert.equal(
...
test("escala hexbin é determinística para vazio, zero, ordem e teto 1/2/5", () => {
  ...
  assert.deepEqual(
    Object.fromEntries(forward.entries.map((entry: DynamicFixture) => [entry.cellId, entry.radiusRatio])),
    Object.fromEntries(reverse.entries.map((entry: DynamicFixture) => [entry.cellId, entry.radiusRatio])),
  );
});
...
test("hexbin separa gradiente de valor real do estado binário", () => {
  ...
  const byId = new Map<string, DynamicFixture>(scale.entries.map((entry: DynamicFixture) => [entry.cellId, entry]));
  const palette = occupancyHexPalette.getOccupancyHexPalette(
    "light",
    "#1267C4",
...
test("máximos por cenário usam recortes civis ordenados de hoje, 12 meses e 4 anos", () => {
  ...
  assert.deepEqual(
    ranges.hourly.buckets.map((bucket: DynamicFixture) => bucket.getHours()),
...
test("eixo máximo horário mantém 24 posições, futuro vazio e substitui a hora aberta", () => {
  ...
  const metrics = new Map(
    range.buckets.map((bucket: DynamicFixture, hour: number) => [
      occupancyAggregateValidation.occupancyAggregateBucketKey(
        bucket,
        "hour",
      ),
...
test("catálogo de ocupação publica os três comparativos máximos por cenário", () => {
  const cardIds = viewPreferences
    ...
    .cards.map((card: DynamicFixture) => card.id);

  assert.deepEqual(
    cardIds.filter((id: string) => id.startsWith("occupancy_scenario_max_")),
    [
...
test("catálogo multiempresa do Master é particionado antes da validação", () => {
  ...
  assert.deepEqual(
    partition.scopedRows.map((worker: DynamicFixture) => worker.id),
    ["worker-selected"],
  );
  assert.deepEqual(
    partition.foreignRows.map((worker: DynamicFixture) => worker.id),
    ["worker-jwt"],
  );
  ...
  assert.deepEqual(
    scopedScenarioRows.map((scenarioRow: DynamicFixture) => scenarioRow.id),
    ["scenario-selected"],
...
test("análise por período envia empresa e abort signal em toda consulta", () => {
  ...
  assert.ok(
    (source.match(/companyScopeId,\s*controller\.signal/g)?.length ?? 0) >= 3,
...
test("mês aberto compara somente horas fechadas equivalentes do ano anterior", () => {
  ...
  const july = model.yearOverYearMonths.find((row: DynamicFixture) => row.month === 6);
  const comparison =
...
test("base parcial sobreposta ganha série própria no comparativo anual", () => {
  ...
  const selected2025 = comparison.rows.find(
    (row: DynamicFixture) => row.year === 2025 && !row.baselineOnly,
  );
  ...
  const rowKeys = comparison.rows.map(
    (row: DynamicFixture) => `${row.year}:${row.baselineOnly ? "comparison-baseline" : "selected-period"}`,
  );
  assert.equal(
...
test("mês civil sem eventos permanece zero certificado", () => {
  ...
  const july = model.yearOverYearMonths.find((row: DynamicFixture) => row.month === 6);
  const currentYear = model.yearRows.find((row: DynamicFixture) => row.year === 2026);
...
test("cada série heatmap de ocupação possui visualMap no motor ECharts", () => {
  ...
  try {
    ...
    assert.deepEqual(
      visualMap.map((entry: DynamicFixture) => entry.seriesIndex),
      [0, 1],
    );
    assert.deepEqual(
...
test("catálogo de paletas de ocupação oferece variações persistíveis e seguras", () => {
  ...
  for (const palette of occupancyColorPalettes.OCCUPANCY_COLOR_PALETTES) {
    ...
    palette.colors.forEach((color: string) => assert.match(color, /^#[0-9A-F]{6}$/));
  }
  assert.equal(
...
test("intervalo de ocupação persiste isolado por empresa e usuário sem bloquear a UI", () => {
  ...
  const localStorage = {
    ...
    setItem(key: string, value: DynamicFixture) {
      values.set(key, value);
    },
  };
  browserFixture.window = { localStorage };
  try {
...
test("configurações de ocupação ficam isoladas por empresa, usuário e cenário", () => {
  ...
  const previousWindow = browserFixture.window;
  const previousCustomEvent = browserFixture.CustomEvent;
  browserFixture.CustomEvent = class CustomEvent {
    type: string;
    detail: unknown;
  ...
  try {
    ...
    else browserFixture.window = previousWindow;
    if (previousCustomEvent === undefined) delete browserFixture.CustomEvent;
    else browserFixture.CustomEvent = previousCustomEvent;
  }
});
...
test("configurações antigas reaparecem por visão sem atravessar usuário ou empresa", () => {
  ...
  const scopedKey = (baseKey: string, companyId?: string, userId?: string, viewId?: string) =>
  ...
  try {
    ...
    const precedenceScope = ["company-a", "user-a", "scenario-a"] as const;
    ...
    assert.equal(
      periodAnalysisWidgets
        ...
        .some((widget: DynamicFixture) => widget.id === "analysis-legacy-custom"),
      true,
    );
...
test("Visões recupera presets e video walls de namespaces anteriores", () => {
  ...
  try {
    ...
    assert.deepEqual(
      videoWall.loadSavedLiveViews(companyId, userId).map((view: DynamicFixture) => view.id),
      [savedView.id],
    );
    assert.deepEqual(
      videoWall.loadVideoWallProfiles(companyId, userId).map((wall: DynamicFixture) => wall.id),
      [videoWallProfile.id],
    );
    assert.deepEqual(
      widgetViewPresets
        .loadWidgetViewPresets("live", companyId, userId)
        .map((view: DynamicFixture) => view.id),
      [preset.id],
    );
    assert.ok(
...
test("presets legados de ocupação migram isolados por superfície", () => {
  ...
  const preset = (id: string, scopeId: DynamicFixture) => ({
    createdAt: "2026-08-11T12:00:00.000Z",
    id,
    isDefault: false,
    name: id,
...
test("filtros 22 e 23 reutilizam exatamente os mesmos meses horários", () => {
  ...
  assert.deepEqual(
    throughDay22.map(({ key, from, to }: DynamicFixture) => [
      key,
      from.getTime(),
      to.getTime(),
    ]),
    throughDay23.map(({ key, from, to }: DynamicFixture) => [
      key,
      from.getTime(),
      to.getTime(),
    ]),
...
test("plano horário preserva lacunas e respeita o limite exclusivo mensal", () => {
  ...
  assert.deepEqual(
    queries.map((query: DynamicFixture) => query.key),
    ["2025-07", "2026-01"],
  );
...
test("cache mensal só entrega horas dentro do corte solicitado", () => {
  ...
  assert.deepEqual(
    rows.map((row: DynamicFixture) => [row.bucket, row.total]),
...
test("consultas 22 e 23 reutilizam o mesmo snapshot HTTP do dia 22", async () => {
  ...
  try {
    ...
    const day22First = through22.find((row: DynamicFixture) =>
      row.bucket.startsWith("2026-07-22"),
    );
    const day22Second = through23.find((row: DynamicFixture) =>
      row.bucket.startsWith("2026-07-22"),
    );
    ...
    assert.deepEqual(
      through22.map((row: DynamicFixture) => row.total),
      [22],
    );
    assert.deepEqual(
      through23.map((row: DynamicFixture) => row.total),
      [22, 23],
    );
  } finally {
...
test("dia aberto e fechado conservam o mesmo snapshot mensal", async () => {
  ...
  try {
    ...
    assert.equal(requests.length, 1);
    assert.deepEqual(
      first.map((row: DynamicFixture) => [row.bucket, row.total]),
      [
...
test("duração de ocupação classifica buckets conservadoramente e une segmentos", () => {
  ...
  assert.deepEqual(
    summary.segments.map((segment: DynamicFixture) => ({
...
test("duração não inventa precisão em buckets de transição ou ausentes", () => {
  ...
  assert.deepEqual(
    summary.segments.map((segment: DynamicFixture) => segment.state),
    ["transition", "unknown"],
  );
});
...
test("resumos temporais unem sequência confirmada entre consultas", () => {
  ...
  assert.deepEqual(
    combined.segments.map((segment: DynamicFixture) => [segment.state, segment.seconds]),
    [
      ["occupied", 180],
      ["free", 60],
    ],
...
test("agregado diário mantém buckets certificados e sinaliza lacunas sem inventar zero", () => {
  ...
  assert.deepEqual(
    coverage.missingBuckets.map((bucket: DynamicFixture) => [
      bucket.getFullYear(),
      bucket.getMonth(),
      bucket.getDate(),
    ]),
...
test("descoberta de ocupação mantém snapshot como fallback do catálogo novo", async () => {
  ...
  const request = async (path: string) => {
    if (path === "/occupancy/areas") {
      const error: Error & { status?: number } = new Error("Rota ainda não publicada.");
      error.status = 404;
      throw error;
    }
    if (path === "/cameras") {
...
test("fallback de ocupação não perde snapshots quando linhas aninhadas retornam 404", async () => {
  ...
  const request = async (path: string) => {
    ...
    if (path === "/cameras/camera-a/line-counts") {
      const error: Error & { status?: number } = new Error("Rota indisponível no escopo selecionado.");
      error.status = 404;
      throw error;
    }
    if (path.startsWith("/occupancy?")) {
...
test("eixo horário de ocupação mantém 24 slots sem inventar futuro", () => {
  ...
  assert.deepEqual(
    points.slice(11).map((point: DynamicFixture) => [point.average, point.current, point.minimum, point.peak]),
...
test("comparativo semanal preserva um bucket temporal exato por semana exibida", () => {
  ...
  const localParts = (date: DynamicFixture) => [
    date.getFullYear(),
    date.getMonth(),
    date.getDate(),
  ];
...
test("comparativo semanal permanece único e monotônico nas viradas de mês", () => {
  ...
  comparisonBuckets.slice(1).forEach((bucket: DynamicFixture, index: number) => {
...
test("comparativo horário preserva instantes DST repetidos sem duplicar o destino", () => {
  ...
  try {
    ...
    assert.deepEqual(normalTarget.map((date: DynamicFixture) => date.getHours()), [0, 1, 2]);
    ...
    assert.deepEqual(fallbackTarget.map((date: DynamicFixture) => date.getHours()), [0, 1, 1, 2]);
    assert.deepEqual(
      fallbackTarget.map((date: DynamicFixture) => date.toISOString()),
      [
...
test("snapshot curto de minutos substitui a mesma hora no gráfico minuto", () => {
  ...
  assert.deepEqual(
    rows
      ...
      .sort(
        ([leftHour, leftMinute]: DynamicFixture, [rightHour, rightMinute]: DynamicFixture) =>
...
test("bucket detalhado substitui integralmente identidades do agregado", () => {
  ...
  assert.deepEqual(
    rows.map((row: DynamicFixture) => [row.line_count_id, row.total]),
    [["line-entry", 60]],
  );
});
...
test("identidades com separador não colidem durante o rollup", () => {
  ...
  assert.deepEqual(
    rows.map((row: DynamicFixture) => row.total).sort((left: DynamicFixture, right: DynamicFixture) => left - right),
    [2, 3],
  );
});
...
test("rollup horário recompõe cada dia em uma única passagem", () => {
  ...
  assert.deepEqual(
    rows
      .map((row: DynamicFixture) => [row.bucket, row.total])
      .sort(([left]: DynamicFixture, [right]: DynamicFixture) => left.localeCompare(right)),
    [
      ["2026-07-22", 9],
      ["2026-07-23", 6],
    ],
...
test("horas repetidas no fim do DST permanecem buckets distintos", () => {
  ...
  assert.equal(rows.length, 2);
  assert.equal(new Set(rows.map((row: DynamicFixture) => row.bucket)).size, 2);
  assert.equal(rows.reduce((sum: number, row: DynamicFixture) => sum + row.total, 0), 5);
});
...
test("agrupamento horário preserva o fuso da empresa e transições DST", () => {
  for (const [timezone, probe] of [
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", "tsx", "tests/timezone-reconciliation-probe.mts", probe],
      {
        cwd: projectRoot,
        encoding: "utf8",
...
test("preferências de widget isolam payloads inválidos e consolidam ids duplicados", () => {
  ...
  for (const invalidPayload of [null, "inválido", 42, { widget: true }]) {
    ...
    assert.deepEqual(
      normalized.map(({ id, visible }: DynamicFixture) => ({ id, visible })),
  ...
  assert.deepEqual(
    normalized.map(({ id }: DynamicFixture) => id),
...
test("preferência por widget normaliza legado, todos e seleção personalizada", () => {
  ...
  const byId = new Map<string, DynamicFixture>(preferences.map((preference: DynamicFixture) => [preference.id, preference]));
...
test("preset faz round-trip da composição de cenários por widget", () => {
  ...
  try {
    ...
    const storedById = new Map<string, DynamicFixture>(
      storedPreset.snapshot.preferences.map((preference: DynamicFixture) => [
        preference.id,
        preference,
      ]),
    );
...
test("grade Bento preserva seis níveis e oferece KPI mínimo compacto", () => {
  ...
  assert.deepEqual(
    legacy.map((item: DynamicFixture) => [item.widthLevel, item.heightLevel]),
    [
      [1, 1],
      [3, 3],
      [5, 5],
  ...
  migratedFourByThree.forEach((preference: DynamicFixture, index: number) => {
  ...
  assert.deepEqual(
    viewPreferences.normalizeCardPreferences(
      "live",
      migratedFourByThree,
      migratedFourByThree.map((preference: DynamicFixture) => preference.id),
    ),
    migratedFourByThree,
  ...
  const allDimensionPreferences = cardLayoutSizing.CARD_LAYOUT_LEVELS.flatMap(
    (widthLevel) =>
      cardLayoutSizing.CARD_LAYOUT_LEVELS.map((heightLevel: DynamicFixture) => ({
        heightLevel,
  ...
  assert.deepEqual(
    allDimensions.map((preference: DynamicFixture) => [
...
test("visões legadas 4x3 migram por armazenamento, preset e API sem mudar o layout", () => {
  ...
  try {
    ...
    assert.deepEqual(
      loaded.map(({ widthLevel, heightLevel }: DynamicFixture) => [widthLevel, heightLevel]),
      expectedLevels,
    );
    ...
    assert.deepEqual(
      persisted.map(({ widthLevel, heightLevel }: DynamicFixture) => [widthLevel, heightLevel]),
      expectedLevels,
    );
    assert.deepEqual(
      persisted.map(({ size, height }: DynamicFixture) => [size, height]),
    ...
    assert.deepEqual(
      presetRoundTrip.map(({ widthLevel, heightLevel }: DynamicFixture) => [
        widthLevel,
        heightLevel,
      ]),
      expectedLevels,
...
test("miniatura Bento aplica uma única escala à largura, linhas e espaçamento", () => {
  ...
  for (const span of [5, 6, 9, 12, 15, 18, 24]) {
    ...
    const previewPixels: number = widgetBentoPreviewLayout.resolveWidgetBentoSpanPixels(
      span,
      geometry.rowHeight,
      geometry.gap,
    );
...
test("composição de Contagem isola A, B, A+B e seleção vazia por widget", () => {
  ...
  const buildModel = (selectedScenarios: DynamicFixture) =>
...
test("granularidades minuto, semana e mês preservam o recorte configurado", () => {
  ...
  assert.deepEqual(
    minutePoints.map((point: DynamicFixture) => point.total),
    [2, 3],
  );
  assert.deepEqual(
    weekPoints.map((point: DynamicFixture) => point.total),
    [4, 5],
  );
  assert.deepEqual(
    monthPoints.map((point: DynamicFixture) => point.total),
    [6, 7],
  );
});
...
test("heatmap de Contagem preserva toda linha selecionada nos dias 1 a 3", () => {
  ...
  const totals = new Map(
    points
      .filter((point: DynamicFixture) => point.total > 0)
      .map((point: DynamicFixture) => [`${point.day}-${point.hour}`, point.total]),
  );
...
test("heatmap de Contagem no Ao Vivo usa tema da tela e light na exportação", () => {
  ...
  const buildOperationalHeatmapOption = loadStandaloneFunction(
    ...
    "buildOperationalHeatmapOption",
    {
      ...
      formatNumber: (value: DynamicFixture) => String(value),
      holidayCategoryIndexesForMonth:
        chartCalendarAxis.holidayCategoryIndexesForMonth,
      hourRangeLabel: (hour: number) => `${hour}h–${hour + 1}h`,
...
test("heatmaps de Ocupação usam contorno suave nas séries presente e ausente", () => {
  const buildHeatmapOption = loadStandaloneFunction(
    ...
    "buildHeatmapOption",
    {
      ...
      truncateLabel: (value: DynamicFixture) => String(value),
    },
  );
  const expectations = {
    dark: {
...
test("heatmap de Demographics usa tema da tela e light na exportação", () => {
  ...
  for (const theme of ["light", "dark"] as const) {
    ...
    heatmapColors.forEach((color: string, index: number) => {
    ...
    assert.equal(series.data.reduce((total: number, value: DynamicFixture) => total + value[3], 0), 4);
  }
...
test("heatmap consolida as duas ocorrências da hora repetida em uma célula", () => {
  ...
  const repeatedHour = points.filter(
    (point: DynamicFixture) => point.day === 1 && point.hour === 1,
  );
    ...
    test(`Análises ${kind} ${description} não colore feriados ou finais de semana`, () => {
      ...
      try {
        for (const browserTimeZone of ["UTC", "America/Sao_Paulo"]) {
          ...
          assert.equal(
            series.reduce(
              (total: number, item: DynamicFixture) => total + item.data.reduce(
                (sum: number, value: DynamicFixture) => sum + (value ?? 0),
                0,
              ),
              0,
            ),
  ...
  test(`Análises ${kind} diário preserva o feriado na categoria civil correta`, () => {
    ...
    try {
      for (const browserTimeZone of ["UTC", "America/Sao_Paulo"]) {
        ...
        assert.equal(model.hasData, true, context);
        const { series, xAxis } = model.option;
        const holidayBands = series.flatMap((item: DynamicFixture) =>
          (item.markArea?.data ?? []).filter(
            ([start]: DynamicFixture) => start.name === "Independência do Brasil",
          ),
        );
...
test("Total do dia e Tendência 7 x 30 usam exatamente a mesma fonte horária", () => {
  ...
  const selectedDay = trend.table?.rows.find(
    (row: DynamicFixture) => row.date === "22/07",
  );
...
test("Tendência 7 x 30 mantém identidade, ordem e legenda idênticas no Ao Vivo e Análises", () => {
  ...
  for (const [context, option] of [
    ...
    assert.deepEqual(
      option.series.map((series: DynamicFixture) => series.name),
      expectedNames,
    ...
    try {
      ...
      for (const trendSeries of [average7, average30]) {
        const rawSeries = option.series.find(
          (series: DynamicFixture) => series.name === trendSeries.name,
        );
...
test("legendas de Ocupação apontam somente para séries visíveis e com cores canônicas", () => {
  ...
  for (const option of options) {
    ...
    const legendNames = (option.legend?.data ?? []).map((item: DynamicFixture) =>
      typeof item === "string" ? item : item.name,
    );
    for (const name of legendNames) {
      const matchingSeries = option.series.filter(
        (series: DynamicFixture) => series.name === name,
      );
      assert.equal(
...
test("o bucket do dia 22 é invariável ao avançar a âncora para o dia 23", () => {
  ...
  const row22 = anchor22.table?.rows.find((row: DynamicFixture) => row.date === "22/07");
...
test("janeiro mantém o mesmo valor ao fechar e avançar para fevereiro", () => {
  ...
  for (const kind of ["year_monthly", "year_accumulated"]) {
    ...
    const closedJanuary = atJanuaryClose.table?.rows.find(
      (row: DynamicFixture) => row.month === "Jan",
    );
...
test("mês parcial anual ignora horas posteriores à data consultada", () => {
  ...
  assert.equal(
    model.table?.rows.find((row: DynamicFixture) => row.month === "Jan")?.value,
    5,
  );
});
...
test("mês aberto compara somente horas fechadas equivalentes do ano anterior", () => {
  ...
  const accumulatedVariationSeries = accumulatedOption.series.find(
    (series: DynamicFixture) => series.name === "Variação acumulada 2026/2025",
  );
  const reportAssets =
...
test("comparativo acumulado soma apenas meses cobertos nos dois anos", () => {
  ...
  model.yearOverYearMonths = model.yearOverYearMonths.map((row: DynamicFixture) =>
    row.month === 0
...
test("mês fechado sem eventos entra como zero na média certificada", () => {
  ...
  const currentYear = model.yearRows.find((row: DynamicFixture) => row.year === 2026);
  const february = model.yearOverYearMonths.find((row: DynamicFixture) => row.month === 1);
...
test("comparativo anual não injeta meses fora do período selecionado", () => {
  ...
  const variationSeries = monthlyOption.series.find(
    (series: DynamicFixture) => series.name === "Variação 2026/2025",
  );
  const baselineSeries = monthlyOption.series.find(
    (series: DynamicFixture) => series.name === "2025 (base comparável)",
  );
...
test("detalhes horários do relatório ignoram baseline e horas após o corte", () => {
  ...
  assert.equal(
    model.directionalHours.reduce((sum: number, row: DynamicFixture) => sum + row.total, 0),
    3,
  );
  assert.equal(
    model.accessHours.reduce((sum: number, row: DynamicFixture) => sum + row.total, 0),
    3,
  );
...
test("totais por local usam as câmeras do escopo sem deslocar o dia", () => {
  ...
  const withCamera = (bucket: DynamicFixture, lineId: DynamicFixture, total: number, cameraId: DynamicFixture) => ({
  ...
  assert.equal(rows.find((row: DynamicFixture) => row.scope === "Portão A")?.total, 7);
  assert.equal(rows.find((row: DynamicFixture) => row.scope === "Portão B")?.total, 4);
  assert.equal(
    model.insights?.find((item: DynamicFixture) => item.label === "Total combinado")?.value,
    "11",
  );
});
...
test("widgets mensais ficam ancorados na data escolhida sem incluir o dia seguinte", () => {
  ...
  for (const kind of ["ranking", "rose"]) {
    ...
    assert.equal(
      model.table?.rows.find((row: DynamicFixture) => row.scenario === entryScenario.name)
        ?.total,
      809,
  ...
  const scenarioRow = table.table?.rows.find(
    (row: DynamicFixture) => row.scenario === entryScenario.name,
  );
...
test("acumulado por cenário replica o recorte exato do Ao Vivo", () => {
  ...
  assert.equal(rows.find((row: DynamicFixture) => row.scenario === "Entrada")?.total, 7);
  assert.equal(rows.find((row: DynamicFixture) => row.scenario === "Saída")?.total, 4);
  assert.equal(model.insights?.find((item: DynamicFixture) => item.label === "Total")?.value, "11");
  assert.equal(model.hasData, true);
});
...
test("visão antiga de acumulado é migrada sem perder a configuração", () => {
  ...
  try {
    ...
    const migrated = widgets.find(
      (widget: DynamicFixture) => widget.id === "legacy-live-cumulative",
    );
    ...
    assert.ok(
      widgets.some((widget: DynamicFixture) => widget.kind === "hourly_occupancy"),
    ...
    if (previousWindow === undefined) {
      ...
      browserFixture.window = previousWindow;
    }
  }
});
...
test("catálogo de Análises contém todos os modelos visuais do Ao Vivo", () => {
  const kinds = new Set(
    periodAnalysisWidgets
      ...
      .map((widget: DynamicFixture) => widget.kind),
  );
...
test("Ao Vivo, Análises, Relatórios e comparativos certificam o fuso antes da consulta", () => {
  ...
  for (const surface of ["analysis", "live", "reports"] as const) {
    assert.match(
      sources[surface],
...
test("plano horário mantém o mesmo snapshot mensal ao avançar do dia 22 para o 23", () => {
  ...
  assert.deepEqual(
    through22.map(({ key, from, to }: DynamicFixture) => [key, from.getTime(), to.getTime()]),
    through23.map(({ key, from, to }: DynamicFixture) => [key, from.getTime(), to.getTime()]),
  );
});
...
test("perfil e saldo horários materializam somente a janela detalhada", () => {
  ...
  assert.equal(
    profile.table?.rows.find((row: DynamicFixture) => row.hour === "10h")?.total,
    10,
  ...
  assert.equal(
    balance.table?.rows.some((row: DynamicFixture) => String(row.period).includes("10/01")),
    false,
  );
});
...
test("comparação extrema agrupa séries excedentes em Outros sem perder total", () => {
  ...
  assert.equal(
    model.insights?.find((insight: DynamicFixture) => insight.label === "Total combinado")
      ?.value,
    "100",
  );
});
...
test("tendência e acumulado amostram só a visualização e preservam o fechamento", () => {
  ...
  const visualDaily = trendSeries.find(
    (series: DynamicFixture) => series.name === "Volume diário",
  )?.data;
  const visualCurrent = cumulativeSeries.find(
    (series: DynamicFixture) => series.name === "Período selecionado",
  )?.data;
...
test("tabela consolidada mantém no mês os mesmos membros agrupados no dia", () => {
  ...
  const other = visibleRows.find((row: DynamicFixture) =>
    String(row.scenario).startsWith("Outros"),
  );

  assert.equal(visibleRows.find((row: DynamicFixture) => row.scenario === "Cenário 0")?.month, 101);
...
test("cache minuto a minuto baixa o dia uma vez e reconcilia a janela móvel", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      caughtUp.map((row: DynamicFixture) => row.total).sort((left: DynamicFixture, right: DynamicFixture) => left - right),
      [2, 6, 9],
    );
  } finally {
    globalThis.fetch = originalFetch;
...
test("loader agregado certifica o teto em qualquer granularidade", async () => {
  ...
  assert.deepEqual(rows.map((row: DynamicFixture) => row.total), [1, 2]);
});
...
test("loader horário encaminha todas as subdivisões à fila e preserva cache e completude", async () => {
  ...
  assert.equal(rows.reduce((total: number, row: DynamicFixture) => total + row.total, 0), 4);
...
test("fontes horárias e diárias compartilham o mesmo teto de quatro requisições físicas", async () => {
  ...
  let peak = 0;
  let calls = 0;
  const request = createCountingHistoryRequestQueue(async (path: string) => {
    active += 1;
    calls += 1;
...
test("consultas dos dias 22 e 23 reutilizam uma resposta mensal e recortam localmente", async () => {
  ...
  try {
    ...
    assert.equal(requests.length, 1);
    assert.deepEqual(through22.map((row: DynamicFixture) => row.total), [22]);
    assert.deepEqual(through23.map((row: DynamicFixture) => row.total), [22, 23]);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("loader horário recupera o início do mês quando a API limita cada resposta a 1.000 linhas", async () => {
  ...
  const sourceRows: AggregateFixtureRow[] = [];
  ...
  const requests: { from: Date; to: Date; returned: number; source: number }[] = [];
  globalThis.fetch = async (input) => {
  ...
  try {
    ...
    const dayTotals = new Map(
      dayRows
        .filter((row: DynamicFixture) => row.line_count_id === "line-entry")
        .map((row: DynamicFixture) => [row.bucket, row.total]),
    );
    ...
    assert.equal(
      heatmap.find((point: DynamicFixture) => point.day === 1 && point.hour === 0)?.total,
      2,
    );
    assert.deepEqual(
      Array.from(
        new Set(
          heatmap
            .filter((point: DynamicFixture) => point.total > 0)
            .map((point: DynamicFixture) => point.day),
        ),
      ),
...
test("comparativo anual substitui somente o mês aberto pelas horas fechadas", () => {
  ...
  assert.equal(
    model.yearRows.find((row: DynamicFixture) => row.year === 2025)?.months[6],
    400,
...
test("histórico anual ao vivo só consulta cards visíveis, uma vez por dia e em meses completos", () => {
  ...
  partitions.slice(1).forEach((partition: DynamicFixture, index: number) => {
    assert.equal(
...
test("planejador do Ao Vivo consulta somente fontes dos widgets visíveis", () => {
  ...
  const build = ({
    ...
    openScenarioComparisonWidgetIds?: string[];
...
test("Ao Vivo recorta respostas compartilhadas antes de certificar cada widget", () => {
  ...
  assert.deepEqual(
    selectRows(sharedRows, definition, definition.to).map((row: DynamicFixture) => row.total),
    [20, 30],
  );
...
function loadStandaloneFunction(
  relativePath: string,
  functionName: string,
  ...
  return loadedModule.exports as ExtractedFunction;
}
...
function pickOccupancy(point: OccupancyFixturePoint) {
  return {
...
function scenario(id: string, name: string, lineCountId: string, actionMultiplier: number) {
  return {
    active: true,
    company_id: "company",
    id,
...
function aggregateRow(
  ...
  lineCountId: string,
  total: number,
  ..._ignoredLegacyArguments: unknown[]
): AggregateFixtureRow {
  // Preserve the original JavaScript helper's treatment of surplus arguments.
  void _ignoredLegacyArguments;
  return {
    bucket,
    camera_id: "camera",
    line_count_id: lineCountId,
...
function normalizeAggregateRows(rows: AggregateFixtureRow[]) {
  return rows
    .map((row) => ({
      bucket: row.bucket,
...
function analysisWidget(kind: string, overrides: Record<string, unknown> = {}) {
  return {
    baseline: "previous_period",
...
function scenarioFixture(id: string, name: string) {
  return {
    active: true,
    company_id: "company-a",
    id,
...
test("todos os gráficos exibem valores permanentes inclinados a 45 graus", () => {
  ...
  const cumulativeValueSeries = cumulativeInteractiveOption.series.filter(
    (series: DynamicFixture) => series.type === "bar" && series.silent !== true,
  );
  ...
  for (const option of annualOptions) {
    for (const lineSeries of option.series.filter(
      (series: DynamicFixture) => series.type === "line" && series.lineStyle?.color,
    )) {
      assert.equal(
    ...
    originalValueSeries.forEach((series: DynamicFixture) => {
    ...
    const valueSeries = converted.series.filter(
      (series: DynamicFixture) => series.type === "line" && series.silent !== true,
    );
  ...
  const preservedLayoutCallback = applyChartTypePreference(
    {
      series: [
        {
          data: [10, 20],
          labelLayout: ({ dataIndex }: DynamicFixture) => ({ dx: dataIndex + 1 }),
          type: "bar",
        },
      ],
  ...
  const currentYearAverage = currentYearOption.series.find(
    (series: DynamicFixture) => series.type === "line",
  );
...
test("exportação executiva separa gráficos e dados sem reduzir tabelas extensas", () => {
  ...
  assert.deepEqual(
    reportTablesForMode(
      ...
      "complete",
      [{ table: sharedTable }],
    ).map((table: DynamicFixture) => table.rows[0].period),
    ["Setembro"],
  );
  ...
  const mockDoc = {
    getTextWidth: (text: DynamicFixture) => String(text).length * 4.2,
    setFont: () => {},
    setFontSize: () => {},
  };
  const columns = [
  ...
  assert.ok(bands.every((band: DynamicFixture) => band.columns[0].key === "indicator"));
  assert.deepEqual(
    bands.flatMap((band: DynamicFixture) => band.columns.slice(1).map((column: DynamicFixture) => column.key)),
...
test("widgets do Ao Vivo respondem à largura real sem ocultar texto essencial", () => {
  ...
  const section = (source: DynamicFixture, from: DynamicFixture, to: DynamicFixture) =>
...
test("réguas principais preservam controles compactos sem forçar overflow em telas estreitas", () => {
  ...
  const toolbarSection = (source: DynamicFixture, label: DynamicFixture, endMarker: DynamicFixture) => {
...
test("heatmap dias x meses preserva grade civil, zero e escopo do cenário", () => {
  ...
  const cell = (month: number, day: DynamicFixture) =>
    model.dayMonthHeatmapCells.find(
      (item: DynamicFixture) => item.month === month && item.day === day,
    );
  ...
  const unavailableSeries = option.series.find(
    (series: DynamicFixture) => series.name === "Sem dia civil fechado",
  );
  const certifiedSeries = option.series.find(
    (series: DynamicFixture) => series.name === "Fluxo certificado",
  );
  ...
  assert.ok(
    certifiedSeries.data.some(
      ([dayIndex, month, total]: DynamicFixture) =>
  ...
  assert.deepEqual(
    option.visualMap.map(({ seriesIndex }: DynamicFixture) => seriesIndex),
    [0, 1],
...
test("heatmap meses x anos limita quatro anos e exporta cobertura legível", () => {
  ...
  const unavailableSeries = option.series.find(
    (series: DynamicFixture) => series.name === "Mês fora do período",
  );
  const certifiedSeries = option.series.find(
  ...
  assert.deepEqual(
    monthYear.value.table.rows.find(
      ({ month, year }: DynamicFixture) => month === "Set" && year === 2026,
    ),
    {
...
test("card anual usa janeiro até o corte aplicado, não apenas os dias selecionados", () => {
  ...
  assert.equal(model.table?.rows.find((row: DynamicFixture) => row.month === "Jan")?.value, 100);
  assert.equal(model.table?.rows.find((row: DynamicFixture) => row.month === "Jul")?.value, 700);
