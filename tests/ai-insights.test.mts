...
const loadTypeScriptModule = createModuleLoader(projectRoot, { compilerOptions: { esModuleInterop: true }, mocks: { "server-only": {} } });
...
const browserGlobal = globalThis as unknown as { window?: { localStorage: Pick<Storage, "getItem" | "setItem" | "removeItem"> } };
...
test("leitor limita o corpo UTF-8 completo por bytes", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof Error && "code" in error && error.code === "invalid_body",
  );
});
...
test("ação abre a última análise, gera sob demanda e exporta o IA Advisor", () => {
  ...
  const integrations: [string, RegExp][] = [
...
test("helper de migração lê e remove configuração legada com escopo exato", () => {
  ...
  browserGlobal.window = { localStorage: storage };
  ...
  try {
    ...
    if (previousWindow === undefined) delete browserGlobal.window;
    else browserGlobal.window = previousWindow;
  }
});
...
// Schema tests intentionally mutate these wire fixtures into invalid payloads.
function validRequest(): RuntimeFixture {
  return {
    snapshot: {
      version: 1,
      binding: {
...
function validConfigurationUpdate(): RuntimeFixture {
  return {
    apiKey: "opaque-server-credential-1234567890",
...
function validReport(): RuntimeFixture {
  return {
    id: "analysis-1",
...
function validAdminConfiguration(): RuntimeFixture {
  return {
    companyId: "company-a",
    configured: true,
    constraints: "Respeitar capacidade, orçamento e segurança.",
...
function validStatus(overrides: Record<string, unknown> = {}): RuntimeFixture {
  return {
    allowedModels: ["gpt-default", "gpt-secondary"],
    available: true,
    configured: true,
...
function validInsights(): RuntimeFixture {
  return {
