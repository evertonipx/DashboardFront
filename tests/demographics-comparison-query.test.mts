...
test("seis horas exigem releitura integral e substituem correções, zeros e ausência", async () => {
  ...
  for (const rows of [[row(0, 0)], []] as const) {
    ...
    assert.ok(summary.gender.every(({ percentage }: RuntimeFixture) => percentage === null));
...
function fixture() {
  const state: RuntimeFixture = { now: Date.parse("2026-09-11T13:00:00Z"), rows: [], failure: null };
  ...
  const loadRange = async (from: RuntimeFixture, to: RuntimeFixture, signal: RuntimeFixture, options: RuntimeFixture) => {
    requests.push({ from: from.getTime(), to: to.getTime(), revalidate: options.revalidate });
    signal.throwIfAborted();
    if (state.failure) throw state.failure;
    return aggregate(state.rows.filter(({ bucket }: RuntimeFixture) => Date.parse(bucket) >= from.getTime() && Date.parse(bucket) < to.getTime()));
  };
  const query = (minutes: RuntimeFixture, changes: Record<string, RuntimeFixture> = {}) => loadDemographicComparisonAggregation({
...
function aggregate(rows: RuntimeFixture) { return aggregateDemographicBuckets(rows, { timeZone: "America/Sao_Paulo" }); }
function row(minute: RuntimeFixture, count: RuntimeFixture) {
...
function deferred() { let resolveRequest: RuntimeFixture; const promise = new Promise<RuntimeFixture>((resolve) => { resolveRequest = resolve; }); return { promise, resolve: resolveRequest }; }
...
function load(path: string, bindings: Record<string, RuntimeFixture> = {}): RuntimeFixture {
  ...
  new Function("module", "exports", "require", ...Object.keys(bindings), compiled)(loadedModule, loadedModule.exports,
    (name: RuntimeFixture) => name.startsWith("@/") ? load(`${name.slice(2)}.ts`) : require(name), ...Object.values(bindings));
