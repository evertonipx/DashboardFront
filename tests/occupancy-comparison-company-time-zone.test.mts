...
const require = createRequire(import.meta.url), ts: typeof import("typescript") = require("typescript"), modules = new Map();
...
test("hora repetida no DST mantém ambas as ocorrências e exige cobertura das duas", () => {
  withBrowserZone("UTC", () => {
    ...
    const metrics = new Map(range.buckets.map((bucket: RuntimeFixture, index: number) => [bucket.getTime(), { average: index + 1, peak: index + 1, minimum: index + 1 }]));
...
test("mês e ano permanecem calendários civis e o pico aberto usa o ano da empresa", () => {
  withBrowserZone("UTC", () => {
    ...
    assert.ok(points.slice(0, -1).every((point: RuntimeFixture) => point.value === null));
  });
});
