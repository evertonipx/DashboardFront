...
function load(path: string): RuntimeFixture {
  ...
  compile(readFileSync(resolve(root, path), "utf8"), { module: loaded, exports: loaded.exports, require: (name: RuntimeFixture) => name.startsWith("@/") ? load(`${name.slice(2)}.ts`) : require(name) });
...
const windowFor = (values: RuntimeFixture) => build({ timeZone: "America/Sao_Paulo", mode: "previous-period", startInput: "2026-09-10", endInput: "2026-09-10", cutoff: new Date("2026-09-11T03:00:00Z"), ...values });
...
test("mês fechado compara mês fechado, incluindo fevereiro bissexto e fevereiro curto", () => {
  for (const [year, leapDays] of [[2024, 29], [2026, 28]] as const) {
...
function harness(overrides: Record<string, RuntimeFixture> = {}) {
  ...
  const releases: RuntimeFixture[] = [];
  const comparisonRequestRef: { current: RuntimeFixture } = { current: null };
  ...
  const run = () => compile(`return (${effect.arguments[0].getText(ast)});`, {
    ...
    userFacingErrorMessage: (error: RuntimeFixture) => error.message,
    loadPartitionedDemographicAggregation: (options: RuntimeFixture) => {
      requests.push(options);
      if (state.pending) return new Promise<RuntimeFixture>((resolve, reject) => releases.push({ resolve, reject }));
      return state.error ? Promise.reject(state.error) : Promise.resolve({ total: 17 });
    },
    setComparisonState: (value: RuntimeFixture) => { state.comparisonState = value; states.push(value); },
  })();
  const flush = async () => { const callbacks = [...timers.values()]; timers.clear(); callbacks.forEach((callback) => callback()); await new Promise<RuntimeFixture>((resolve) => setImmediate(resolve)); };
...
test("falha no período principal encerra o carregamento do comparativo e apresenta o erro", () => {
  const current = (overrides: Record<string, RuntimeFixture> = {}) => compile(`const comparisonError = ${comparisonPresentationExpressions.comparisonError}; return {loading: ${comparisonPresentationExpressions.comparisonLoading}, error: comparisonError};`, {
...
test("Refresh e tick do Ao Vivo bloqueiam baseline até a publicação da mesma versão e corte principal", async () => {
  ...
  const ready = (bindings: RuntimeFixture) => compile(`return (${comparisonReadyExpression});`, bindings);
