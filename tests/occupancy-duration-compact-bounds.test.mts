...
const require = createRequire(import.meta.url), ts: typeof import("typescript") = require("typescript"), echarts = require("echarts"), modules = new Map();
...
for (const theme of ["light", "dark"]) test(`eixo percentual 0–100% cabe inteiro em gráficos compactos (${theme})`, () => {
  ...
  for (const [width, height] of [[320, 190], [300, 128], [768, 230]] as const) {
    const chart = echarts.init(null, null, { renderer: "svg", ssr: true, width, height });
    try {
      chart.setOption(option);
      const labels: RuntimeFixture[] = [];
      chart.getViewOfComponentModel(chart.getModel().getComponent("yAxis")).group.traverse((element: RuntimeFixture) => {
