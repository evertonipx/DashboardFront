...
function settings(overrides: Record<string, RuntimeFixture> = {}, dimension = "gender") {
  return { ...defaultDemographicPresentation(dimension), ...overrides };
}

function allData(option: RuntimeFixture) {
  return option.series.flatMap((series: RuntimeFixture) => series.data);
}
...
for (const [type, orientation] of presets) {
  test(`${type}/${orientation}: mostra apenas gêneros identificados, rebasa percentuais e preserva contagens brutas`, () => {
    ...
    assert.deepEqual(data.map((point: RuntimeFixture) => point.key), ["Woman", "Man"]);
    assert.equal(data.reduce((total: RuntimeFixture, point: RuntimeFixture) => total + point.percentage, 0), 100);
    assert.equal(option.aria.enabled, true);
    assert.equal(option.aria.decal.show, false);
    for (const original of identifiedItems) {
      const point = data.find((entry: RuntimeFixture) => entry.key === original.key);
...
test("ordenação é estável e não troca categoria, cor, rótulo ou percentual", () => {
  ...
  for (const palette of DEMOGRAPHICS_PALETTES) {
    ...
    for (const [order, expected] of [
      ...
      assert.deepEqual(sorted.map((entry: RuntimeFixture) => entry.key), expected);
      for (const point of sorted) {
        assert.deepEqual(point, natural.find((entry: RuntimeFixture) => entry.key === point.key));
      }
    }
  }
...
test("cores padrão rosa/azul e emojis opcionais preservam a identidade dos gêneros visíveis", () => {
  ...
  for (const emojis of [false, true]) {
    ...
    data.forEach((point: RuntimeFixture, index: number) => {
...
for (const type of ["pie", "donut", "half-donut", "rose"]) {
  test(`${type}: legenda, rótulo e tooltip usam o percentual canônico, não params.percent`, () => {
    ...
    assert.deepEqual(option.legend.data.map((entry: RuntimeFixture) => entry.name), identifiedItems.map((entry) => entry.label));
...
test("sem gêneros identificados, a base vazia permanece null e não ganha participação artificial", () => {
  ...
  for (const [type, orientation] of presets) {
    ...
    assert.deepEqual(allData(option).map((point: RuntimeFixture) => point.key), ["Woman", "Man"]);
    for (const series of option.series) for (const datum of series.data) {
      ...
      const original = source.find(({ key }) => key === datum.key);
      assert.ok(original, "a categoria renderizada deve existir na origem");
      assert.equal(datum.observed, original.observed);
      const params = { data: datum, value: datum.value, percent: 50 };
      assert.equal(series.label.formatter(params), "");
...
for (const type of ["stacked"]) {
  test(`${type}: compacto mantém nove percentuais legíveis na legenda e restaura o layout ao ampliar`, () => {
    ...
    assert.ok(option.media.some((entry: RuntimeFixture) => entry.query?.maxWidth === 400));
    assert.ok(option.media.some((entry: RuntimeFixture) => entry.query?.maxHeight === 160));
    const chart = echarts.init(null, null, { renderer: "svg", ssr: true, width: 900, height: 320 });
    try {
      chart.setOption({ ...option, animation: false });
      for (const [width, height] of [[251, 125], [674, 125], [251, 125]] as const) {
        ...
        assert.ok(actual.series.every((series: RuntimeFixture) => series.label.show === false));
        ...
        for (const node of chart.getZr().storage.getDisplayList().filter((item: RuntimeFixture) => item.type === "tspan")) {
      ...
      assert.ok(enlarged.series.every((series: RuntimeFixture) => series.label.show === true));
      ...
      if (type !== "stacked") assert.ok(enlarged.series.every((series: RuntimeFixture) => series.labelLine.show === true), "linhas de referência da pizza devem reaparecer ao ampliar");
      assert.ok(enlarged.series.every((series: RuntimeFixture) => series.emphasis.label.show === true), "o estado de destaque não pode manter o label oculto do compacto");
      ...
      assert.deepEqual(allData(enlarged).map((datum: RuntimeFixture) => datum.count), source.map((datum) => datum.count));
    } finally { chart.dispose(); }
  });
}
...
function legendText(chart: RuntimeFixture) {
  const model = chart.getModel().getComponent("legend");
  const view = chart.getViewOfComponentModel(model);
  const values: RuntimeFixture[] = [];
  view.group.traverse((element: RuntimeFixture) => {
...
test("trocar de formato na mesma instância não conserva meia circunferência, roseType ou séries antigas", () => {
  ...
  try {
    for (const type of ["half-donut", "pie", "rose", "donut", "stacked", "pie", "half-donut"]) {
      ...
      assert.deepEqual(allData(actual).map((datum: RuntimeFixture) => datum.key), identifiedItems.map((datum) => datum.key));
      assert.deepEqual(allData(actual).map((datum: RuntimeFixture) => datum.count), identifiedItems.map((datum) => datum.count));
      if (type !== "stacked") {
        const data = chart.getModel().getSeriesByIndex(0).getData();
    ...
    for (const [dimension, source] of Object.entries(compositionSources)) for (const theme of ["light", "dark"]) {
      ...
      try {
        for (const size of compositionSizes) {
          ...
          const svg = chart.renderToSVGString();
          const actual = chart.getOption();
          const names = sourceData.map((datum: RuntimeFixture) => datum.name);
          assert.deepEqual(actual.legend[0].data.map((datum: RuntimeFixture) => typeof datum === "string" ? datum : datum.name).filter(Boolean), names, `${context}: ordem da legenda deve seguir dados`);
          ...
          for (const element of chart.getZr().storage.getDisplayList().filter((item: RuntimeFixture) => item.type === "tspan" && !item.ignore && !item.invisible)) {
          ...
          for (const group of legendView.getContentGroup().childrenRef()) {
            ...
            const expected = sourceData.find((datum: RuntimeFixture) => datum.name === name);
            assert.ok(expected, `${context}: legenda inesperada ${name}`);
            const icon = group.childrenRef().find((element: RuntimeFixture) => element.type !== "text" && element.style?.fill && !element.ignore && !element.invisible && element.style.opacity !== 0);
...
function semanticDatum(datum: RuntimeFixture) {
  return {
  ...
  for (const { id: palette } of DEMOGRAPHICS_PALETTES) for (const theme of ["light", "dark"]) {
    for (const type of ["pie", "donut", "half-donut", "rose"]) {
      ...
      try {
        ...
        assert.deepEqual(allData(adapted).map((datum: RuntimeFixture) => datum.itemStyle.color), allData(option).map((datum: RuntimeFixture) => datum.itemStyle.color));
      } finally { chart.dispose(); }
    }
  }
});

function formattedPercentage(value: RuntimeFixture) {
  return `${new Intl.NumberFormat("pt-BR", { maximumFractionDigits: 2 }).format(value)}%`;
}

function assertRadialLabels(chart: RuntimeFixture, sourceData: RuntimeFixture, context: string) {
  ...
  const outsideBounds: RuntimeFixture[] = [];
  ...
  for (let index = 0; index < data.count(); index++) {
    const datum = data.getRawDataItem(index);
    const expected = sourceData.find((candidate: RuntimeFixture) => candidate.key === datum.key);
    ...
    const spans = label?.childrenRef().filter((span: RuntimeFixture) => span.type === "tspan" && !span.ignore && !span.invisible) ?? [];
    const positive = typeof expected.percentage === "number" && expected.percentage > 0;
    if (!positive) {
      assert.ok(!label || label.ignore || !spans.some((span: RuntimeFixture) => span.style?.text), `${context}: zero/ausência não podem pintar percentual`);
      continue;
    }
    assert.ok(label && !label.ignore && !label.invisible, `${context}: percentual de ${datum.name} precisa estar visível sem hover`);
    assert.equal(spans.map((span: RuntimeFixture) => span.style?.text ?? "").join(""), formattedPercentage(expected.percentage), `${context}: valor canônico precisa estar associado à fatia ${datum.name}`);
    ...
    if (position === "inside" || position === "inner") {
      ...
      assert.ok(guide.shape.points.length >= 2 && guide.shape.points.every((point: RuntimeFixture) => point.every(Number.isFinite)), `${context}: linha-guia deve possuir coordenadas válidas`);
      assert.ok(guide.shape.points.some((point: RuntimeFixture, pointIndex: number, points: RuntimeFixture) => pointIndex > 0 && Math.hypot(point[0] - points[pointIndex - 1][0], point[1] - points[pointIndex - 1][1]) > 1), `${context}: linha-guia não pode ser degenerada`);
...
function assertTextInsideSector(bounds: RuntimeFixture, layout: RuntimeFixture, context: string) {
...
function globalBounds(element: RuntimeFixture) {
  const bounds = element.getBoundingRect().clone();
  bounds.applyTransform(element.getComputedTransform());
  return bounds;
}

function rectanglesOverlap(left: RuntimeFixture, right: RuntimeFixture) {
...
function load(path: string): RuntimeFixture {
  ...
  new Function("module", "exports", "require", javascript)(loaded, loaded.exports, (name: RuntimeFixture) =>
