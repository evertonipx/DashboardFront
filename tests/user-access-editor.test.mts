...
function execute(path: string) {
  ...
  const compiledModule: { exports: RuntimeFixture } = { exports: {} };
...
function option(family: string, capability: string, overrides = {}) {
...
const group = (groups: RuntimeFixture, id = "product:counting") => groups.find((candidate: RuntimeFixture) => candidate.id === id);
...
test("editor sempre identifica as três telas dos três produtos e os menus solicitados", () => {
  ...
  assert.deepEqual(groups.slice(0, 3).map((item: RuntimeFixture) => item.label), ["Contagem", "Ocupação", "Demográfico"]);
  for (const item of groups.slice(0, 3)) {
    assert.deepEqual(item.options.map((row: RuntimeFixture) => row.label), ["Ao vivo", "Análises", "Relatórios"]);
    assert.ok(item.options.every((row: RuntimeFixture) => row.disabled && !row.checked && row.permissionKeys.length === 0));
  }
  assert.deepEqual(group(groups, "menus").options.map((row: RuntimeFixture) => row.label), [
...
test("permissão legada mostra três seleções vinculadas ao mesmo grant, sem inventar permissões", () => {
  ...
  assert.ok(item.options.every((row: RuntimeFixture) => row.linked && row.checked && !row.disabled));
...
test("permissões granulares mantêm os controles de cada tela independentes", () => {
  ...
  assert.deepEqual(item.options.map((row: RuntimeFixture) => row.checked), [false, true, false]);
  assert.ok(item.options.every((row: RuntimeFixture) => !row.linked && !row.disabled));
...
test("acesso conjunto existente não finge que uma tela pode ser revogada individualmente", () => {
  ...
  const live = item.options.find((row: RuntimeFixture) => row.id === "live");
...
test("seleção do grupo limpa também grants específicos cobertos por gestão", () => {
  ...
  assert.ok(item.options.slice(0, 3).every((row: RuntimeFixture) => row.disabled && row.checked));
...
test("recursos de dois módulos não compartilham a seleção", () => {
  ...
  const rows = item.options.filter((row: RuntimeFixture) => row.label === "Workers");
  assert.deepEqual(rows.map((row: RuntimeFixture) => row.checked), [true, false]);
...
test("Admin seleciona todos os acessos reais dos módulos habilitados sem conceder o módulo desabilitado", () => {
  ...
  assert.ok(group(groups).options.every((row: RuntimeFixture) => row.checked));
  assert.ok(group(groups, "product:demographics").options.every((row: RuntimeFixture) => row.checked));
  assert.ok(group(groups, "product:occupancy").options.every((row: RuntimeFixture) => !row.checked && row.disabled));
});
...
test("auditoria conserva a restrição exclusiva do Superadmin enquanto não houver mudança autorizada", () => {
  ...
  const audit = item.options.find((row: RuntimeFixture) => row.id === "audit");
...
test("gestão vinculada ao módulo não apresenta as telas liberadas como desmarcadas", () => {
  ...
  assert.ok(item.options.every((row: RuntimeFixture) => row.checked && row.disabled));
  assert.ok(item.options.every((row: RuntimeFixture) => /recursos de gestão/.test(row.description)));
  assert.deepEqual(userAccessEditorPermissionKeys(item), []);
});
