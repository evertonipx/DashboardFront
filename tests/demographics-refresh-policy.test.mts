...
function compile(source: RuntimeFixture, bindings: Record<string, RuntimeFixture> = {}) {
...
function load(path: string, bindings: Record<string, RuntimeFixture> = {}): RuntimeFixture {
...
const policy: typeof import("../lib/demographics-refresh-policy.ts") = load("lib/demographics-refresh-policy.ts");
...
test("backoff cresce em 1/2/4/5 minutos, fica limitado e não modifica o estado anterior", () => {
  let previous: import("../lib/demographics-refresh-policy.ts").DemographicRetryState | null = null;
  for (let index = 0; index < 15; index++) {
    const now = Date.UTC(2026, 8, 11, 12) + index * minute;
    const before: import("../lib/demographics-refresh-policy.ts").DemographicRetryState | null = previous ? structuredClone(previous) : null;
...
test("dependências do ticker não incluem apresentação, formato, disposição ou resumo", () => {
  const dependencies = timerEffect.arguments[1].elements.map((node: import("typescript").Node) => node.getText(ast)).sort();
...
test("hook real observa visibilidade/online/offline sem rede ou polling, com SSR inativo", () => {
  ...
  const fakeNavigator: { onLine: boolean | undefined } = { onLine: true };
  const hook = load("components/app/use-demographics-page-active.ts", {
    require: (name: RuntimeFixture) => {
      assert.equal(name, "react");
      return { useSyncExternalStore: (subscribe: RuntimeFixture, snapshot: RuntimeFixture, serverSnapshot: RuntimeFixture) => { store = { subscribe, snapshot, serverSnapshot }; return snapshot(); } };
    },
...
function ticker(overrides: Record<string, RuntimeFixture> = {}) {
  const now = Date.UTC(2026, 8, 11, 12, 0, 10);
  const state: RuntimeFixture = { surface: "live", pageActive: true, preferencesReady: true, hasVisibleWidgets: true, now, clock: new Date(now - minute), clockWrites: 0, changes: [], ...overrides };
  const refs: Record<string, RuntimeFixture> = { pendingLiveAggregationRef: { current: null }, comparisonRequestRef: { current: null }, liveRetryRef: { current: null } };
  const timers = new Map();
  let timerId = 0;
  const fakeWindow: RuntimeFixture = { setTimeout: (callback: (...args: RuntimeFixture[]) => RuntimeFixture, delay: RuntimeFixture) => { const id = ++timerId; timers.set(id, { callback, at: state.now + delay }); return id; }, clearTimeout: (id: RuntimeFixture) => timers.delete(id) };
  const fakeDocument = { visibilityState: "visible" }, fakeNavigator: { onLine: boolean | undefined } = { onLine: true };
  ...
  function setup() {
    return compile(`return (${timerEffect.arguments[0].getText(ast)})();`, {
      ...
      setClock: (update: RuntimeFixture) => {
        state.clockWrites++;
  ...
  function advance(to: RuntimeFixture) {
    state.now = to;
