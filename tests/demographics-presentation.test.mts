...
test("rosa e azul é primeira e todas as paletas de Ocupação são reutilizadas sem alteração", () => {
  ...
  assert.equal(new Set(palettes.map((palette: RuntimeFixture) => palette.id)).size, palettes.length);
  for (const palette of palettes) {
    assert.ok(palette.label && palette.description);
    assert.ok(palette.colors.every((color: string) => /^#[\da-f]{6}$/i.test(color)));
...
test("nomes demográficos descrevem os pares reais sem renomear paletas compartilhadas ou IDs salvos", () => {
  ...
  assert.deepEqual(presentation.DEMOGRAPHICS_PALETTES.map(({ id }: RuntimeFixture) => id), Object.keys(expected));
  ...
  for (const { id: palette, label: originalLabel } of presentation.DEMOGRAPHICS_PALETTES) {
    ...
    for (const [id, dimension] of [["demographics_gender_mix", "gender"], ["demographics_age_gender_pyramid", "age-gender"]] as const) {
...
test("emojis são opcionais, preservam textos e não decoram categorias desconhecidas", () => {
  for (const [label, key, dimension] of [["Mulher", "Woman", "gender"], ["0-2", "0-2", "age"], ["Neutro", "neutral", "emotion"], ["Não identificado", "unknown", "age-gender"]] as const) {
...
test("demographics nulo recupera defaults seguros e normalizar novamente é idempotente", () => {
  ...
  normalized.forEach((card: RuntimeFixture) => assert.deepEqual(card.demographics, presentation.defaultDemographicPresentation(dimensions[card.id])));
...
test("save/load real usa o mesmo namespace, outbox e isolamento por empresa, usuário e visão", (t) => {
  ...
  const events: RuntimeFixture[] = [];
  fixtureGlobals.window = {
    localStorage: {
      getItem: (key: string) => stored.get(key) ?? null,
      setItem: (key: string, value: RuntimeFixture) => stored.set(key, String(value)),
      removeItem: (key: string) => stored.delete(key),
      key: (index: number) => [...stored.keys()][index] ?? null,
      get length() { return stored.size; },
    },
    dispatchEvent: (event: RuntimeFixture) => { events.push(event); return true; },
  };
  t.after(() => { if (previousWindow === undefined) delete fixtureGlobals.window; else fixtureGlobals.window = previousWindow; });
  const id = "demographics_gender_mix";
  ...
  scopes.forEach((scope, index) => {
    ...
    assert.ok(mutations.some((mutation: RuntimeFixture) => mutation.key === key && !mutation.deleted));
