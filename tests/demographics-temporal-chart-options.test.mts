...
function load(path: string): RuntimeFixture {
  ...
  new Function("module", "exports", "require", output)(loaded, loaded.exports, (name: RuntimeFixture) => name.startsWith("@/") ? load(`${name.slice(2)}.ts`) : require(name));
...
let plannerCalls = 0;
temporal.buildDemographicTemporalPlan = (...arguments_: RuntimeFixture[]) => { plannerCalls++; return originalPlan(...arguments_); };
...
visible.visibleDemographicDistribution = (...arguments_: RuntimeFixture[]) => { genderRebaseCalls++; return originalVisibleDistribution(...arguments_); };
...
test("perfil horário mantém 24 horas e pondera contagens antes de calcular o percentual", () => {
  ...
  const row = model.table.rows.find((row: RuntimeFixture) => row.period === "10h" && row.category === "Mulher");
  ...
  assert.equal(model.table.rows.filter((row: RuntimeFixture) => row.period === "10h").reduce((total: RuntimeFixture, row: RuntimeFixture) => total + row.percentage, 0), 100);
});
...
test("selecionar uma categoria não renormaliza o denominador nem altera dados de origem", () => {
  ...
  assert.deepEqual(selected.table.rows, all.table.rows.filter((row: RuntimeFixture) => row.category === "Mulher"));
  assert.equal(selected.table.rows.find((row: RuntimeFixture) => row.period === "10h").percentage, 78.57);
...
test("todos os widgets e formatos de gênero mostram apenas identificados e declaram a base dos percentuais", () => {
  ...
  for (const id of ids) for (const chartType of ["bar", "area", "line", "heatmap"]) for (const metric of ["percentage", "count"]) {
    ...
    assert.deepEqual([...new Set(model.table.rows.map((row: RuntimeFixture) => row.category))], ["Mulher", "Homem"]);
    const percentages = model.table.columns.filter((column: RuntimeFixture) => /percentage$/.test(column.key) && !column.key.startsWith("change"));
    assert.ok(percentages.every((column: RuntimeFixture) => /entre gêneros identificados/.test(column.label)));
  ...
  assert.equal(summary.gender.find((item: RuntimeFixture) => item.key === "unknown").count, 10);
  assert.deepEqual(summary, snapshot);
});
...
test("arredondamento de gêneros identificados soma 100% sem absorver unknown nem renormalizar filtros", () => {
  ...
  const observed = model.table.rows.filter((row: RuntimeFixture) => row.total !== null);
  assert.deepEqual(observed.map((row: RuntimeFixture) => row.percentage), [3.13, 96.87]);
  assert.equal(observed.reduce((sum: RuntimeFixture, row: RuntimeFixture) => sum + row.percentage, 0), 100);
  assert.ok(observed.every((row: RuntimeFixture) => row.total === 1032));
  const onlyWoman = modelFor("demographics_daily_evolution", { categoryKeys: ["Woman", "unknown"] }, { summary: rounding });
  assert.equal(onlyWoman.table.rows.find((row: RuntimeFixture) => row.total !== null).percentage, 3.13);
});
...
test("idade e emoção conservam as detecções de gênero desconhecido e os percentuais originais", () => {
  for (const dimension of ["age", "emotion"]) {
    ...
    const row = model.table.rows.find((row: RuntimeFixture) => row.period === "09/09/2026" && row.category === label);
    ...
    assert.ok(model.table.columns.some((column: RuntimeFixture) => column.key === "percentage" && column.label === "Participação no intervalo (%)"));
    const comparison = modelFor("demographics_period_comparison", { dimension });
    assert.equal(comparison.table.rows.find((row: RuntimeFixture) => row.category === label).current_percentage, 5);
  }
});
...
test("zero observado, lacuna e futuro permanecem distintos no gráfico e na tabela nos dois temas", () => {
  for (const theme of ["light", "dark"]) {
    ...
    const observedZero = hourly.table.rows.find((row: RuntimeFixture) => row.period === "12h" && row.category === "Mulher");
    const missing = hourly.table.rows.find((row: RuntimeFixture) => row.period === "09h" && row.category === "Mulher");
    ...
    assert.ok(countSeries.data.some((datum: RuntimeFixture) => datum.value[2] === 0));
    assert.ok(hourly.option.series[0].data.some((datum: RuntimeFixture) => datum.value[2] === -1));
    const current = modelFor("demographics_gender_timeline", {}, { to: "2026-09-10T03:00:00Z", now: "2026-09-09T13:30:00Z", theme });
    const future = current.option.series[0].data.find((datum: RuntimeFixture) => datum.periodLabel === "11:00");
    ...
    const empty = heatmap.option.series[0].data.find((datum: RuntimeFixture) => datum.periodLabel === "11:00");
  ...
  for (const id of ["demographics_emotion_hourly", "demographics_age_hourly"]) {
    for (const { id: palette } of DEMOGRAPHICS_PALETTES) {
      for (const metric of ["count", "percentage"]) {
        ...
        assert.deepEqual(dark.option.series.map((series: RuntimeFixture) => series.data), light.option.series.map((series: RuntimeFixture) => series.data));
...
test("sem agregado temporal não inventa uma série temporal a partir do total", () => {
  ...
  assert.equal(model.hasData, false);
  assert.ok(model.table.rows.every((row: RuntimeFixture) => row.count === null && row.percentage === null));
});
...
test("comparativo informa pontos percentuais e não converte diferença em crescimento relativo", () => {
  ...
  const row = model.table.rows.find((row: RuntimeFixture) => row.category === "Mulher");
  ...
  assert.ok(model.table.columns.some((column: RuntimeFixture) => column.key === "change_pp" && column.label.includes("p.p.")));
...
test("comparativo absoluto calcula deltas e evita infinito quando a referência é zero ou inexistente", () => {
  ...
  const woman = count.table.rows.find((row: RuntimeFixture) => row.category === "Mulher");
  assert.equal(count.table.rows.some((row: RuntimeFixture) => row.category === "Não identificado"), false);
  ...
  const man = zero.table.rows.find((row: RuntimeFixture) => row.category === "Homem");
  assert.equal(man.reference_count, 0);
  assert.equal(man.change_count_percentage, null);
  assert.ok(count.table.columns.some((column: RuntimeFixture) => column.key === "change_count_percentage"));
  assert.ok(!count.table.columns.some((column: RuntimeFixture) => column.key === "change_pp"));
  const absent = modelFor("demographics_period_comparison", {}, { comparisonSummary: null });
  assert.ok(absent.table.rows.every((row: RuntimeFixture) => row.reference_count === null && row.reference_percentage === null && row.change_pp === null));
  ...
  for (const theme of ["light", "dark"]) {
    for (const chartType of ["bar", "line", "area"]) {
      for (const palette of ["pink-blue", "cyber"]) {
        ...
        assert.deepEqual(model.option.series.map((series: RuntimeFixture) => series.symbol), ["circle", "diamond"]);
...
test("áreas, linhas e barras de gênero desenham as cores exatas das prévias sem transparência residual", () => {
  ...
  for (const { id: palette } of DEMOGRAPHICS_PALETTES) for (const theme of ["light", "dark"]) {
    ...
    for (const chartType of ["bar", "area", "line"]) {
      ...
      assert.deepEqual(model.option.color, colors);
      model.option.series.forEach((series: RuntimeFixture, index: number) => {
      ...
      try {
        ...
        for (const [index, color] of colors.entries()) {
          ...
          if (chartType === "area") {
            ...
            const valueLabels: RuntimeFixture[] = [];
            chart.getViewOfSeriesModel(seriesModel).group.traverse((element: RuntimeFixture) => {
            ...
            for (const text of valueLabels) {
              ...
              const backgrounds = text.childrenRef().filter((element: RuntimeFixture) => element.type === "rect");
  ...
  for (const dimension of ["age", "emotion"]) {
    ...
    assert.ok(model.option.series.every((series: RuntimeFixture) => series.areaStyle.opacity === 0.32));
    assert.ok(model.option.series.every((series: RuntimeFixture) => series.label.backgroundColor === undefined));
  }
  const oneDay = modelFor("demographics_daily_evolution", { chartType: "area" }, { to: "2026-09-10T03:00:00Z" });
  assert.ok(oneDay.option.series.every((series: RuntimeFixture) => series.label.backgroundColor === undefined));
  assert.deepEqual(summary, snapshot);
});

function colorContrast(first: RuntimeFixture, second: RuntimeFixture) {
  const luminance = (color: string) => {
    const channels = echarts.color.parse(color).slice(0, 3).map((channel: number) => {
...
test("modelo diário respeita agregação por dia e histórico extenso promove hora sem milhares de pontos", () => {
  ...
  const firstWoman = daily.table.rows.find((row: RuntimeFixture) => row.period === "09/09/2026" && row.category === "Mulher");
...
test("modelos idênticos mantêm referências e tema/paleta reutilizam tabelas e dados sem refazer percentuais", () => {
  ...
  for (const chartType of ["line", "bar", "area", "heatmap"]) {
    ...
    for (const context of [{ theme: "dark" }, { theme: "light" }]) {
      ...
      recolored.option.series.forEach((series: RuntimeFixture, index: number) => assert.equal(series.data, first.option.series[index].data));
    }
...
test("resoluções auto/hour/day equivalentes compartilham uma varredura anual e o relógio fechado não invalida", () => {
  ...
  const before = plannerCalls;
  const values = ids.map((id: RuntimeFixture) => modelFor(id, {}, context));
  ...
  for (const now of ["2026-09-11T03:01:00Z", "2026-09-12T03:00:00Z", "2027-09-11T03:00:00Z"]) {
    ids.forEach((id: RuntimeFixture, index: number) => assert.equal(modelFor(id, {}, { ...context, now }), values[index]));
  }
  assert.equal(plannerCalls - before, 1);
...
test("now omitido acompanha o relógio nos minutos abertos e estabiliza somente depois do fim", () => {
  ...
  try {
    ...
    const nextHour = (model: RuntimeFixture) => model.option.series[0].data.find((datum: RuntimeFixture) => datum.periodLabel === "10:00");
...
test("caches temporais são limitados, descartando cortes antigos sem alterar seu resultado", () => {
  ...
  assert.deepEqual(restored.option.series.map((series: RuntimeFixture) => series.data), first.option.series.map((series: RuntimeFixture) => series.data));
});
...
test("cache não modifica agregações congeladas, tabelas, valores ou preferências", () => {
  ...
  first.option.series.forEach((series: RuntimeFixture) => deepFreeze(series.data));
...
function deepFreeze(value: RuntimeFixture) {
...
test("densidade adapta apenas apresentação e preserva todos os dados e tooltips", () => {
  for (const id of ids) {
    ...
    const data = structuredClone(model.option.series.map((series: RuntimeFixture) => series.data));
    for (const size of [{ width: 260, height: 120 }, { width: 1360, height: 320 }]) {
      const option = fit(model, size);
      assert.deepEqual(option.series.map((series: RuntimeFixture) => series.data), data);
      assert.equal(option.tooltip, model.option.tooltip);
      assert.ok(option.series.every((series: RuntimeFixture) => series.label.fontSize >= 9));
...
test("um único dia desenha barras distintas sem alterar a preferência, valores ou formato de heatmap", () => {
  for (const chartType of ["line", "area", "bar"]) {
    ...
    assert.ok(model.option.series.every((series: RuntimeFixture) => series.type === "bar" && !series.stack && !series.areaStyle));
    assert.deepEqual(model.option.series.map((series: RuntimeFixture) => series.data[0].value), [55.56, 44.44]);
    assert.equal(model.table.rows.find((row: RuntimeFixture) => row.category === "Mulher").percentage, 55.56);
  }
  const heatmap = modelFor("demographics_daily_evolution", { chartType: "heatmap" }, { to: "2026-09-10T03:00:00Z" });
  assert.ok(heatmap.option.series.every((series: RuntimeFixture) => series.type === "heatmap"));
});
          ...
          try {
            ...
            if (model.kind === "heatmap") {
              ...
              for (const [seriesIndex, series] of option.series.entries()) {
                ...
                series.data.forEach((datum: RuntimeFixture, index: number) => {
