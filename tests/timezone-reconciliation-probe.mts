...
// Values returned by this isolated CommonJS evaluator cross a runtime-only
// boundary. The deterministic input rows remain explicitly typed below.
...
if (probe === "company-four-hour-offset") {
  ...
  assert.deepEqual(
    occupancy.slice(9, 12).map(({ entries, hour, occupancy: value }: DynamicFixture) => ({
      entries,
      hour,
      occupancy: value,
    })),
  ...
  assert.equal(
    occupancy.findIndex((point: DynamicFixture) => point.entries > 0),
    10,
  ...
  assert.equal(new Set(points.map((point: DynamicFixture) => point.bucket)).size, 2);
  assert.deepEqual(points.map((point: DynamicFixture) => point.total), [2, 3]);
  ...
  assert.equal(points.length, 2);
  assert.deepEqual(points.map((point: DynamicFixture) => point.total), [7, 5]);
...
function aggregateRow(bucket: string, total: number) {
  return {
    bucket,
    camera_id: "camera",
