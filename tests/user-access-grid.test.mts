...
function compileModule(path: string, overrides: Record<string, unknown> = {}) {
...
function findElements(tree: React.ReactNode, predicate: (node: React.ReactElement<RuntimeFixture>) => boolean) {
  const result: React.ReactElement<RuntimeFixture>[] = [];
...
function checkboxes(props: RuntimeFixture) {
  return findElements(inspectGrid(props), (node: RuntimeFixture) => node.type === checkboxModule.Checkbox);
}

function render(props: RuntimeFixture) {
...
test("seleção do grupo reflete nenhuma, parcial e todas as opções editáveis", () => {
  for (const [selectedCount, expected] of [[0, false], [1, "indeterminate"], [3, true]] as const) {
...
test("callbacks informam grupo e opção selecionados sem substituir id por key legado", () => {
  const optionEvents: [string, string, boolean][] = [];
  const groupEvents: [string, boolean][] = [];
  const props = createProps([group(baseOptions.map((option) => ({ ...option, key: "shared-legacy-key" })))], {
    onOptionChange: (...args: [string, string, boolean]) => optionEvents.push(args),
    onGroupChange: (...args: [string, boolean]) => groupEvents.push(args),
  });
...
test("acesso legado conjunto tem aviso único por grupo e não aparece em grupos independentes", () => {
  ...
  const sections = findElements(inspectGrid(props), (node: RuntimeFixture) => node.type === "section");
