...
test("cinco padrões temporais são explícitos e não compartilham arrays mutáveis", () => {
  ...
  ids.forEach((id: RuntimeFixture, index: number) => {
...
test("seleções são limitadas à dimensão, deduplicadas e armazenadas por chave canônica", () => {
  ...
  for (const dimension of ["gender", "age", "emotion"]) {
    ...
    assert.ok(categories.every(({ key, label }: RuntimeFixture) => typeof key === "string" && label));
    assert.ok(categories.length <= 9);
    assert.deepEqual(temporal.normalizeDemographicTemporalSettings({ dimension, categoryKeys: categories.map(({ key }: RuntimeFixture) => key).reverse() }, ids[0]).categoryKeys, []);
  }
...
test("seleções legadas de gênero removem unknown sem esconder os gêneros identificados", () => {
  for (const id of ids) {
    for (const [categoryKeys, expected] of [[['unknown'], []], [['Woman', 'unknown'], ['Woman']], [['Man', 'unknown'], ['Man']], [['unknown', 'Woman', 'Man'], []]] as const) {
...
test("persistência só aceita demographicsTemporal nos cinco IDs e não altera objetos legados", () => {
  assert.ok(ids.every((id: RuntimeFixture) => preferences.getCardMenuDefinition("demographics").cards.some((card: RuntimeFixture) => card.id === id)));
  for (const id of ids) {
...
test("salvar e recarregar usa writer user-grid e mantém escopos empresa/usuário/visão", (t) => {
  const previous = fixtureGlobals.window;
  const storage = new Map();
  fixtureGlobals.window = { localStorage: {
    getItem: (key: string) => storage.get(key) ?? null, setItem: (key: string, value: RuntimeFixture) => storage.set(key, value), removeItem: (key: string) => storage.delete(key),
    key: (index: number) => [...storage.keys()][index] ?? null, get length() { return storage.size; },
  }, dispatchEvent: () => true };
  t.after(() => { if (previous === undefined) delete fixtureGlobals.window; else fixtureGlobals.window = previous; });
  ...
  assert.ok(grid.readUserGridLocalMutations().some((item: RuntimeFixture) => item.key === preferences.getCardViewStorageKey(...scope)));
...
function allElements(node: RuntimeFixture, predicate: RuntimeFixture): RuntimeFixture[] {
...
function select(tree: RuntimeFixture, label: string) {
  return allElements(tree, (element: RuntimeFixture) => element.type === Select && allElements(element, (child: RuntimeFixture) => child.props["aria-label"] === label).length)[0];
}

test("controles reais limpam categorias ao trocar dimensão e respeitam disabled", () => {
  const changes: RuntimeFixture[] = [];
  const tree = Controls({ widgetId: ids[0], value: settings, onChange: (value: RuntimeFixture) => changes.push(value) });
  ...
  const locked = Controls({ widgetId: ids[0], value: settings, disabled: true, onChange: (value: RuntimeFixture) => changes.push(value) });
  assert.ok(allElements(locked, (node: RuntimeFixture) => node.type === Select).every((node: RuntimeFixture) => node.props.disabled));
...
test("categorias vazias significam todas e UI não permite esconder a última categoria", () => {
  const changes: RuntimeFixture[] = [];
  const tree = Controls({ widgetId: ids[0], onChange: (value: RuntimeFixture) => changes.push(value) });
  const boxes = allElements(tree, (node: RuntimeFixture) => node.type === Checkbox);
  assert.equal(boxes.length, 2);
  assert.ok(boxes.every((box: RuntimeFixture) => box.props.checked));
  boxes[0].props.onCheckedChange(false);
  assert.deepEqual(changes[0].categoryKeys, ["Man"]);
  const single = Controls({ widgetId: ids[0], value: { ...temporal.defaultDemographicTemporalSettings(ids[0]), categoryKeys: ["Woman"] }, onChange: (value: RuntimeFixture) => changes.push(value) });
  const selected = allElements(single, (node: RuntimeFixture) => node.type === Checkbox && node.props.checked)[0];
  assert.equal(selected.props.disabled, true);
  selected.props.onCheckedChange(false);
  assert.equal(changes.length, 1);
  allElements(single, (node: RuntimeFixture) => node.type === Button && node.props.children === "Mostrar todas as categorias")[0].props.onClick();
