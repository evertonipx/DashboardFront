...
const source = readFileSync("tools/verify-responsive.mts", "utf8");
const parsed = ts.createSourceFile("verify-responsive.mts", source, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
...
const focusPaintOutsets: (shadow: string, width?: string, offset?: string, style?: string, color?: string) => Record<"top" | "right" | "bottom" | "left", number> = vm.runInNewContext(compiled);
const outsets = (...args: Parameters<typeof focusPaintOutsets>) => ({ ...focusPaintOutsets(...args) });
