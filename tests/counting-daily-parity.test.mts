...
test("agregados civis recentes renovam por hora; histórico não é baixado em cada tick", async () => {
  ...
  const loadNative = standalone("loadCachedRealtimeNativeQuery", {
    ...
    fetchCompleteAggregateRange: async (query: RuntimeFixture) => { calls.push(query); return [{ total }]; },
  });
  const cache = new Map();
  const base = {
...
function widget(kind: RuntimeFixture, scopeMode: RuntimeFixture, scenarioIds: RuntimeFixture) {
...
function data(rows: RuntimeFixture) {
...
function load(path: string) {
  ...
  new Function("exports", "require", "module", output)(loaded.exports, (specifier: string) => specifier.startsWith("@/") ? load(`${specifier.slice(2)}.ts`) : nodeRequire(specifier), loaded);
