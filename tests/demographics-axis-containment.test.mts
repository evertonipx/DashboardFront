...
}).outputText)((value: RuntimeFixture) => value && typeof value === "object" && !Array.isArray(value));
...
function legacyCardDensity(option: RuntimeFixture, kind: RuntimeFixture, { width, height }: { width: number; height: number }) {
...
function casesFor({ width, height, theme, stress }: RuntimeFixture) {
  const cases: RuntimeFixture[] = [];
  for (const dimension of ["gender", "age", "emotion"]) {
    const items = summary[dimension].map((item: RuntimeFixture, index: number) => ({ ...item, label: stress ? `${longName} ${index}` : item.label }));
  ...
  for (const dimension of ["age-gender", "age-emotion"]) {
    ...
    if (stress) {
      ...
      crossing.rows.forEach((row: RuntimeFixture, index: number) => { row.label = `${longName} ${index}`; });
    }
  ...
  for (const id of DEMOGRAPHICS_TEMPORAL_WIDGET_IDS) {
    ...
    if (stress && option.yAxis.type === "category") {
      option.yAxis.data = option.yAxis.data.map((_: RuntimeFixture, index: number) => `${longName} ${index} 😐`);
    }
...
function yAxisTextBounds(chart: RuntimeFixture) {
...
function withChart(option: RuntimeFixture, size: RuntimeFixture, verify: (...args: RuntimeFixture[]) => RuntimeFixture) {
  ...
  test(`eixo Y permanece contido em ${width}×${height}, ${theme}, incluindo nomes longos e números altos`, () => {
    ...
    for (const stress of [false, true]) {
      ...
      for (const { id, source, option } of cases) {
        ...
        const beforeData = structuredClone(option.series.map((series: RuntimeFixture) => series.data));
        assert.deepEqual(option.series.map((series: RuntimeFixture) => series.data), source.series.map((series: RuntimeFixture) => series.data), `${context}: a densidade não pode alterar valores exportados`);
        ...
        withChart(option, { width, height }, (chart: RuntimeFixture) => {
        ...
        assert.deepEqual(option.series.map((series: RuntimeFixture) => series.data), beforeData, `${context}: renderização não pode alterar dados`);
      }
    }
...
test("opções diretas de exportação preservam containment moderno e os mesmos valores da prévia", () => {
  for (const theme of ["light", "dark"]) {
    for (const { id, source, option } of casesFor({ width: 640, height: 320, theme, stress: true })) {
      ...
      assert.deepEqual(source.series.map((series: RuntimeFixture) => series.data), option.series.map((series: RuntimeFixture) => series.data), id);
      withChart(source, { width: 640, height: 320 }, (chart: RuntimeFixture) => {
