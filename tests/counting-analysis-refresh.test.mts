...
function loadModule(relativePath: string) {
  ...
  const localRequire = (name: string) => name === "@/lib/api"
...
function createFixture({ storageFails = false } = {}) {
  ...
  const state: {
    version: number; serverTotal: number | null; dayRequests: number; hourRequests: number;
    errors: string[]; saved: unknown[][]; hold: Promise<void> | null;
  } = { version: 0, serverTotal: 100, dayRequests: 0, hourRequests: 0, errors: [], saved: [], hold: null };
  const rows = (granularity: string) => state.serverTotal === null ? [] : [{
  ...
  const loadDay = standalone("fetchCachedAnalysisDayDataset", {
    ...
    fetchAnalysisDataset: async (granularity: string, requestedRange: RuntimeFixture, companyScopeId: RuntimeFixture, signal: AbortSignal) => {
  ...
  const commit = standalone("commitAnalysisSettings", {
    ...
    savePeriodAnalysisSettings: (...args: unknown[]) => {
      state.saved.push(args);
      if (storageFails) throw new Error("Storage unavailable");
    },
    toast: { error: (message: RuntimeFixture) => state.errors.push(message) },
    requestRef, abortRequest: (controller: RuntimeFixture) => controller.abort(),
    ...
    setQueryVersion: (update: RuntimeFixture) => { state.version = update(state.version); },
  });
  async function load() {
    ...
    ]).then(([day, hour]) => [day.rows, hour].map((rows) => rows.reduce((sum: RuntimeFixture, row: RuntimeFixture) => sum + row.total, 0)));
  }
...
test("solicitação explícita aborta a anterior e uma resposta tardia não restaura dados velhos", async () => {
  ...
  const rejected = assert.rejects(previous, (error) => error instanceof Error && error.name === "AbortError");
  fixture.apply();
