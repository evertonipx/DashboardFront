...
function grant(id: string, slug: string, module = counting, action?: string) {
...
test("permissões genéricas continuam um único pacote, com IDs e slugs reais", () => {
  ...
  const reading = options.find((option: RuntimeFixture) => option.capability === "counting_view");
  assert.equal(reading.surface, null);
  assert.deepEqual(reading.slugs, ["counting_view", "counting_read"]);
  assert.deepEqual(reading.grants.map(({ id }: RuntimeFixture) => id), ["view-id", "read-id"]);
  assert.notEqual(reading.slug, "counting_view");
  assert.equal(options.find((option: RuntimeFixture) => option.capability === "counting_manage").action, "manage");
  assert.deepEqual(catalog, before);
});

test("slug opaco com módulo ativo e leitura explícita conserva o acesso já aceito no runtime", () => {
  for (const [module, family] of [[counting, "counting"], [occupancy, "occupancy"], [demographics, "demographics"]] as const) {
...
test("Demográfico opaco habilita três controles vinculados sem fabricar IDs ou slugs", () => {
  ...
  for (const selected of [false, true]) {
    ...
    const group = buildUserAccessEditorGroups(options, state).find((item: RuntimeFixture) => item.id === "product:demographics");
    assert.equal(group.options.length, 3);
    assert.ok(group.options.every((item: RuntimeFixture) => !item.disabled && item.linked && item.checked === selected));
  ...
  const unavailableGroup = buildUserAccessEditorGroups(unavailable, unavailableState)
    .find((item: RuntimeFixture) => item.id === "product:demographics");
  assert.ok(unavailableGroup.options.every((item: RuntimeFixture) => item.disabled && !item.checked));
...
test("cada superfície real tem opção própria sem fabricar grants para as demais", () => {
  ...
  assert.deepEqual(options.map((option: RuntimeFixture) => option.surface), ["live", "analytics", "reports", "live", "reports"]);
  assert.ok(options.every((option: RuntimeFixture) => option.grants.length === 1 && option.action === "view"));
  assert.equal(options.filter((option: RuntimeFixture) => option.module_id === occupancy.id).length, 1);
});
...
test("aliases CRUD conhecidos são agrupados sem ampliar para catálogo desconhecido ou auditoria", () => {
  ...
  assert.deepEqual(options.map((option: RuntimeFixture) => option.capability), ["cameras_manage", "dashboard_widgets_manage", "views_manage"]);
...
test("o mesmo slug literal em módulos ou IDs distintos é ambíguo para POST por slug", () => {
  ...
  assert.deepEqual(options.map((option: RuntimeFixture) => option.capability), ["counting_view"]);
  assert.deepEqual(resolveUserAccessCatalog([
...
function catalogUser(permission: RuntimeFixture) {
  return {
