...
const colors = [...new Set([
  ...
  ...OCCUPANCY_COLOR_PALETTES.flatMap((entry: RuntimeFixture) => entry.colors),
])];
...
test("somente a cor das células recebe a suavização de 78%, composta em HEX opaco", () => {
  for (const color of colors) {
    ...
    const expected = previous.map((stop) => stop.map((channel: number) => Math.round(255 + (channel - 255) * 0.78)));
    softened.forEach((stop: RuntimeFixture, index: number) => {
...
for (const theme of ["light", "dark"]) {
  ...
  test(`os três mapas de tempo ocupado compartilham a mesma escala sem inverter no ${theme}`, () => {
    ...
    for (const kind of durationCharts.OCCUPANCY_DURATION_INSIGHT_CARD_IDS.filter((id: RuntimeFixture) => id.endsWith("heatmap"))) {
      ...
      withChart(option, (chart: RuntimeFixture) => {
        ...
        const zeroIndex = values.findIndex((point: RuntimeFixture) => point.value[2] === 0);
        assert.ok(zeroIndex >= 0, `${kind}: deve haver zero certificado`);
        assertRgbEqual(fill(chart, 0, zeroIndex), "#FFFFFF");
        const positiveIndex = values.findIndex((point: RuntimeFixture) => point.value[2] > 0);
...
test("Demographics aplica a escala compartilhada por tema e escolhe rótulos pelo fundo real", () => {
  ...
  for (const paletteId of ["pink-blue", ...OCCUPANCY_COLOR_PALETTES.map((entry: RuntimeFixture) => entry.id)]) {
    for (const theme of ["light", "dark"]) {
      ...
      scale.forEach((color: string, index: number) => {
      ...
      withChart(option, (chart: RuntimeFixture) => {
        const values = option.series[0].data;
        assert.equal(values.reduce((total: RuntimeFixture, value: RuntimeFixture) => total + value[3], 0), 2500);
        const zeroIndex = values.findIndex((value: RuntimeFixture) => value[2] === 0);
        const lowIndex = values.findIndex((value: RuntimeFixture) => value[2] === 10);
        const highIndex = values.findIndex((value: RuntimeFixture) => value[2] === 90);
        ...
        values.forEach((value: RuntimeFixture, index: number) => {
...
test("o tema automático do EChart preserva intensidade e rótulos, mas adapta superfícies e outros gráficos", () => {
  ...
  withChart(applyChartTheme(monthly, true), (chart: RuntimeFixture) => {
    assertRgbEqual(fill(chart, 1, 0), "#FFFFFF");
    assertRgbEqual(fill(chart, 1, 1), scale.at(-1));
...
function assertWhiteToDark(scale: RuntimeFixture, context: string) {
  assert.equal(scale[0].toLowerCase(), "#ffffff", `${context}: o mínimo deve ser branco puro`);
  assert.equal(scale.length, 7);
  const values = scale.map((color: string) => {
    assert.match(color, /^#[0-9a-f]{6}$/i);
    return luminance(color);
  });
  values.slice(1).forEach((value: RuntimeFixture, index: number) => assert.ok(value <= values[index], `${context}: intensidade ${index + 1} não pode ficar mais clara`));
...
function originalHeatmapStops(color: string) {
  const source = echarts.color.parse(color).slice(0, 3);
  const mix = (target: RuntimeFixture, weight: RuntimeFixture) => source.map((channel: number, index: number) => Math.round(channel + (target[index] - channel) * weight));
...
function fill(chart: RuntimeFixture, series: RuntimeFixture, index: number) {
  return chart.getModel().getSeriesByIndex(series).getData().getItemGraphicEl(index).style.fill;
}

function withChart(option: RuntimeFixture, verify: (...args: RuntimeFixture[]) => RuntimeFixture) {
...
function standalone(relativePath: string, name: RuntimeFixture, bindings: RuntimeFixture) {
