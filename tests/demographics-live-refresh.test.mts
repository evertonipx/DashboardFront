...
test("releitura integral de seis horas atualiza partições antigas: 7 passa para 11", async () => {
  ...
  assert.equal(updated.temporal.bins.reduce((sum: RuntimeFixture, bin: RuntimeFixture) => sum + bin.total, 0), 11);
...
test("releitura integral substitui resumo positivo por zero explícito e depois ausência", async () => {
  ...
  assert.ok(zero.gender.every(({ percentage }: RuntimeFixture) => percentage === null));
  harness.state.rows = [];
  harness.state.now += sixHours;
...
test("releitura conserva concorrência de duas partições e deduplica consumidores", async () => {
  const harness = makeHarness();
  const options = liveOptions();
  harness.state.onRequest = () => new Promise<RuntimeFixture>((resolveRequest) => setTimeout(resolveRequest, 1));
...
function makeHarness() {
  ...
  const bindings: Record<string, RuntimeFixture> = {
    ...
    MINUTE_MS: 60_000,
    abortRequest: (controller: RuntimeFixture) => controller.abort(),
    apiFetch: async (path: string, options: RuntimeFixture) => {
      ...
      try {
        ...
        return {
          data: state.rows
            .filter(({ bucket }: RuntimeFixture) => Date.parse(bucket) >= from.getTime() && Date.parse(bucket) < to.getTime())
            .map((row: RuntimeFixture) => ({ ...row })),
        };
      } finally {
        state.active -= 1;
      }
...
function loadDemographics(path = "lib/demographics.ts"): RuntimeFixture {
  ...
  new Function("module", "exports", "require", compiled)(loadedModule, loadedModule.exports, (name: RuntimeFixture) =>
    name.startsWith("@/") ? loadDemographics(`${name.slice(2)}.ts`) : require(name));
  return loadedModule.exports;
}

function standalone(name: RuntimeFixture, bindings: RuntimeFixture) {
...
function liveOptions(): RuntimeFixture {
  return {
...
function row(bucket: RuntimeFixture, count: RuntimeFixture) {
