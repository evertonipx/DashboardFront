...
function execute(source: RuntimeFixture, bindings = {}) {
...
function standaloneFunction(path: string, name: string, bindings = {}) {
...
test("report transport deduplicates identical paths and bounds concurrent requests", async () => {
  ...
  let maximum = 0;
  let calls = 0;
  const queue = createCountingHistoryRequestQueue(async (path: string) => {
    calls += 1;
    active += 1;
...
test("aborting a report generation prevents queued requests from reaching the API", async () => {
  ...
  const queue = createCountingHistoryRequestQueue((path: string) => {
    started.push(path);
...
for (const [path, name] of [
  ...
  test(`${name}: sublocation discovery is bounded, scoped and keeps input order`, async () => {
    ...
    let maximum = 0;
    const controller = new AbortController();
    const calls: { route: string; options: { companyScopeId: string; signal: AbortSignal } }[] = [];
    const fetchLocations = standaloneFunction(path, name, {
      createCountingHistoryRequestQueue,
      apiFetch: async (route: RuntimeFixture, options: RuntimeFixture) => {
        active += 1;
        maximum = Math.max(maximum, active);
      ...
      requireSubLocationRows: (rows: RuntimeFixture, companyId: string) => {
        assert.equal(companyId, "company-a");
        return rows;
      },
      filterScopedApiRows: (rows: RuntimeFixture) => rows,
      selectExplicitCompanyScopedRows: (rows: RuntimeFixture, companyId: string) => {
        assert.equal(companyId, "company-a");
        return { rows };
      },
    });
    ...
    assert.equal(maximum, 4);
    assert.deepEqual(rows.map((row: RuntimeFixture) => row.id), locations.map((row) => row.id));
    assert.ok(calls.every(({ options }) => options.companyScopeId === "company-a" && options.signal === controller.signal));
  });
}

function createDayLoader() {
  const calls: { granularity: string; range: { from: Date; to: Date }; companyScopeId: string }[] = [];
  ...
  const load = standaloneFunction(analysisPath, "fetchCachedAnalysisDayDataset", {
    ...
    setAnalysisDayCacheEntry: (cache: RuntimeFixture, key: string, value: RuntimeFixture) => cache.set(key, value),
    pendingAnalysisDayRequestsForCache: (cache: RuntimeFixture) => {
      if (!pending.has(cache)) pending.set(cache, new Map());
      return pending.get(cache);
    },
    fetchAnalysisDataset: async (granularity: string, range: RuntimeFixture, companyScopeId: RuntimeFixture) => {
...
test("a narrower closed-day analysis reuses certified rows without another request", async () => {
  ...
  assert.equal(result.rows.reduce((sum: RuntimeFixture, row: RuntimeFixture) => sum + row.total, 0), 50);
...
function runReportSelectionEffect(bindings: Record<string, unknown>) {
...
test("an actually empty report catalog invalidates completed keys with the dataset", () => {
  const bindings = {
    ...
    setChartData: (value: RuntimeFixture) => assert.deepEqual(value, {}),
  };
  runReportSelectionEffect(bindings);
