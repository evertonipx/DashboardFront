...
test("segmentos cruzando hora e dia mantêm seus estados e segundos", () => {
  ...
  assert.equal(hours.reduce((sum: RuntimeFixture, hour: RuntimeFixture) => sum + hour.expectedSeconds, 0), summary.expectedSeconds);
});
...
test("hora DST repetida recebe 120 minutos reais e não duplica intensidade", () => {
  ...
  const repeated = hours.find((hour: RuntimeFixture) => hour.hour === 1);
  ...
  const cell = model.dayHours.find((cell: RuntimeFixture) => cell.x === 0 && cell.y === 1);
  assert.equal(cell.confirmedOccupiedSeconds / cell.expectedSeconds, 1);
  assert.equal(model.weekHours.find((cell: RuntimeFixture) => cell.x === 6 && cell.y === 1).expectedSeconds, 7_200);
...
test("hora DST pulada não gera duração desconhecida fictícia", () => {
  ...
  const hours = insights.summarizeOccupancyDurationInsightHours(summary, month.timeZone);
  assert.deepEqual(hours.map((hour: RuntimeFixture) => hour.hour), [0, 1, 3]);
  const model = insights.buildOccupancyDurationInsightModel([{ scenarioId: "a", name: "A", hours }], month);
  const skipped = model.dayHours.find((cell: RuntimeFixture) => cell.x === 7 && cell.y === 2);
...
test("partição horária otimizada equivale à projeção de cada minuto em transições IANA", () => {
  ...
  for (const [timeZone, fromIso, minuteCount] of cases) {
    ...
    const actualTotals = new Map(actual.map((hour: RuntimeFixture) => [`${hour.dateKey}|${hour.hour}`, hour.expectedSeconds]));
...
test("ausência, ocupação zero e futuro permanecem distinguíveis", () => {
  ...
  const model = insights.buildOccupancyDurationInsightModel([{ scenarioId: "a", name: "A", hours }], month);
  const zero = model.dayHours.find((cell: RuntimeFixture) => cell.x === 0 && cell.y === 0);
  const missing = model.dayHours.find((cell: RuntimeFixture) => cell.x === 0 && cell.y === 1);
  const partial = model.dayHours.find((cell: RuntimeFixture) => cell.x === 0 && cell.y === 2);
  const future = model.dayHours.find((cell: RuntimeFixture) => cell.x === 0 && cell.y === 3);
...
test("composições por cenário somam segundos sem confundir transição com permanência", () => {
  ...
  for (const collection of [model.dayHours, model.weekHours, model.scenarioHours, model.days]) {
    assert.equal(collection.reduce((sum: RuntimeFixture, item: RuntimeFixture) => sum + item.expectedSeconds, 0), 7_200);
    assert.equal(collection.reduce((sum: RuntimeFixture, item: RuntimeFixture) => sum + item.confirmedOccupiedSeconds, 0), 3_600);
  }
});
...
test("cenário com falha mantém ausência no denominador e nenhuma presença inventada", () => {
  ...
  assert.ok(empty.dayHours.every((cell: RuntimeFixture) => cell.expectedSeconds === 0));
...
function minuteSummary(from: number, states: string[]) {
  const values: Record<string, { average: number; minimum: number; peak: number }> = {
    occupied: { average: 2, minimum: 1, peak: 3 },
    free: { average: 0, minimum: 0, peak: 0 },
    transition: { average: 0.5, minimum: 0, peak: 1 },
  };
  const buckets = states.map((_: RuntimeFixture, index: number) => new Date(from + index * 60_000));
  const metrics = new Map(states.flatMap((state: RuntimeFixture, index: number) => values[state]
