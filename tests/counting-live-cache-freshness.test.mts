...
test("erro ou cancelamento da hora aberta não promove a cobertura dos minutos", async () => {
  for (const abort of [false, true]) {
    ...
    harness.state.onRequest = ({ from }: RuntimeFixture) => {
...
function makeHarness() {
  type Request = { from: Date; to: Date; granularity: string | null; companyScopeId?: string };
  const requests: Request[] = [];
  const state: {
    hour: ReturnType<typeof row>[];
    minute: ReturnType<typeof row>[];
    onRequest: ((request: Request) => void | Promise<void>) | null;
  } = { hour: [], minute: [], onRequest: null };
  ...
  const time: Record<string, RuntimeFixture> = load("lib/aggregate-time.ts");
...
function minuteOptions(now: RuntimeFixture, from: RuntimeFixture, to: RuntimeFixture) {
  return {
    cache: { cacheScope: "", coveredFrom: null, coveredTo: null, rows: [] } as {
      cacheScope: string; coveredFrom: number | null; coveredTo: number | null;
...
function hourOptions() {
  return {
    cacheScope: scope,
    companyScopeId: "tenant-a",
    coverageCache: { cacheScope: "", ranges: [], rows: [] } as {
      cacheScope: string; ranges: { from: Date; to: Date }[];
      rows: ReturnType<typeof row>[]; dayRevision?: string; hourRevision?: string;
    },
...
function date(value: RuntimeFixture) { return new Date(value); }
function total(rows: RuntimeFixture) { return rows.reduce((sum: RuntimeFixture, row: RuntimeFixture) => sum + row.total, 0); }
function requestBounds({ from, to }: RuntimeFixture) { return [from.toISOString(), to.toISOString()]; }
