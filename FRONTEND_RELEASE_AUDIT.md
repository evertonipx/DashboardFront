...
- O bootstrap de sessão reconcilia o mesmo JWT aceito por `/auth/me`: `sub` e `user_id` precisam identificar o mesmo usuário, `company_id` não pode divergir do perfil explícito e `role` somente completa campos omitidos. Claims conflitantes, expirados, ainda não válidos ou com timezone inválido são rejeitados.
